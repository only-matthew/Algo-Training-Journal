...
## 阶段七：知识地图配色修复与洛谷书系题单并入（09-27，v2.1）

用户在 v2.0 正式构建后提出两条已知问题：**「知识地图还需要升级，出现了黑底黑字」**与「增加新的题单」。这一轮的产出全部围绕"口径要能自证"展开 —— 三处问题都是先被量化、再被修复、最后被测试钉住。

### 1. 黑底黑字：一条兼容规则抹掉了整条难度色阶

**先量后改。** 第一步不是看代码而是量颜色：`artifacts/badge-colors.mjs` 把知识地图上出现的每一类徽标注入真实页面，在浅色/深色两套主题下读取最终计算色与 WCAG 对比度。结果一眼定位：

| 徽标 | 浅色主题实测 | 对比度 |
| --- | --- | --- |
| `.roadmap-luogu-difficulty[data-level="8"]`（NOI/NOI+/CTS） | 文字 `rgb(16,35,28)` 落在底色 `rgb(17,24,39)` 上 | **1.08:1** |
| `.roadmap-difficulty[data-difficulty="4"]` | 白字落在 `#eab308` 上 | 1.92:1 |
| 其余洛谷等级 1/6/7 | 深字落在红/蓝/紫上 | 4.15–4.46:1（均低于 4.5） |

根因不是"某处漏写颜色"，而是**同一个色阶被写了两遍**：`src/style.css` 中段定义洛谷 8 级配色，而文件末尾又追加了一条 `.roadmap-luogu-difficulty{color:var(--text);background:var(--surface-soft)}`。后者位置更靠后、特异性相同，于是整条色阶被折叠成同一块灰底；加上 `data-level=8` 原本用近黑 `#111827` 作底色，深色字落在近黑底上就是**1.08:1** —— 这就是用户看到的"黑底黑字"。其余等级则退化成无法区分的灰底，颜色分级失效。

修法是把色阶收敛成**唯一来源**并让对比度可计算：`lib/roadmap-badges.mjs` 定义节点难度（1-10 分五档）与洛谷 8 级两套底色，`foregroundFor()` 按 WCAG 相对亮度自动在深墨/白色之间选前景色，模板把结果内联到徽标上（样式表再想覆盖也打不回来）。为达标做了两处取色调整：难度 9-10 由 `#ef4444` 压到 `#b91c1c`，洛谷的入门/提高+/省选- 由 `#ef4444`/`#3b82f6`/`#a855f7` 压深一档；NOI 档改用 `#475569`（原来的近黑底在深色主题里与页面底色糊在一起）。样式表里两套色阶的 22 条硬编码色一并删除，只留排版与「浅底深字」兜底。

顺带把五个掌握度状态色也改成由模块下发 `--evidence-light` / `--evidence-dark` 两个自定义属性，样式表只负责用 `color-mix` 算底色 —— 未登记的状态自动回落中性配色，不会再出现"加了新状态却没有对应颜色"的静默失效。

**验证过程中又暴露了两件事，都已处理**：

| 发现 | 处置 |
| --- | --- |
| 把测量从"注入合成标记"改成"直接测真实页面"后，发现 `.tag-chip`（4.37:1）与 `.review-chip.todo`（4.39:1）这两类 12px 小字仍差一点不达 AA —— 它们分别被 `final.css` 的 `--muted`/`--hard` token 覆盖 | 新增 `--tag-chip-ink:#57655f`（5.56:1）、`final.css` 的 `--hard` 改 `#a63c39`（5.37:1）、`--medium` 改 `#8f5404` |
| `scripts/preview-ui.mjs` 不发缓存头，浏览器用启发式缓存留住旧 `style.css`：重建后截图与测量仍是旧样式，而带 `?v=` 的脚本请求又绕过 `no-store`，造成"探针说没改、文件里明明改了"的假象 | 预览服务统一返回 `no-store, no-cache, must-revalidate` |

最终实测（4 个真实页面 × 2 套主题，490 个徽标去重成 30 种）：**严重 0 项、低于 AA 0 项**，同一徽标在两套主题下配色完全一致。`artifacts/contrast-audit.mjs` 的全站逐元素扫描同样是严重级 0。


### 2. 洛谷书系题单：26 个题单 712 道题，按算法标签归并

用户给出洛谷三个官方书系题单：**《算法竞赛进阶指南》（李煜东，14 个题单）/《算法竞赛试炼场：洛谷 300 题精析》（罗勇军，8 个）/《算法竞赛实战笔记》（4 个）**，并要求"直接按照算法标签合并"。

**先探路再动手**：题单目录页（`/training/list?type=book.*`）与标签字典（`/_lfe/tags`）匿名可读，但**题单详情页对匿名请求一律 401「请先登录」**（`UserUnloginException`）；`/problem/list?trainingId=` 之类的参数会被静默忽略，返回全站题单。唯一可行路径是带登录态的 Cookie —— 而 `workers/services/luogu-page.mjs` 已经有一套经过验证的 C3VK 挑战握手，脚本复用它即可。凭证只从环境变量 `LUOGU_COOKIE` 读取，不落盘、不进仓库。

`scripts/fetch-luogu-training.mjs` 因此做成四步链路：目录页 → 题单详情页（从正文 `<ol><li><a href="/problem/…">题号 - 名称</a>` 取清单，**只认带链接的行**，避免把题单说明区里的纯文本题号混进来）→ 题目页（`lentille-context` 取官方题名/难度/标签 id）→ 标签字典（id → 中文名，再经 `lib/luogu-tag-map.mjs` 归一到站内标签）。支持断点续抓（`--skip-existing` 只跳过已抓到题名的题，失败项会被自动重试）。**实测 712 道唯一题目全部抓取成功、0 失败。**

**归属按标签而不是按书的章节**（`lib/curriculum-book-problems.mjs`）。第一版实现按"命中标签个数"排序，结果暴露了一个真实缺陷：`DP` 同时挂在 dp-intro 与 dp-tree-graph 上，于是「DP+背包」的采药被泛化的 dp-intro 抢走，dp-linear 几乎收不到题。改成**按标签独特性加权**（权重 = 1 / 该标签所属节点数）后，`背包`（只挂 dp-linear）权重 1 胜过 `DP` 的 1/2，采药回到 dp-linear。一道题可同时并入最多 3 个节点（`SPECIFICATION` 本就允许"同一道题可支持多个相关节点"）；标签缺失约 4% 的题不猜归属，直接丢弃并计入统计输出。

题目按标签归一化与题号去重：712 道唯一题目归入 1322 条节点记录（覆盖全部 39 个节点），书系题数 1138。节点题单上限 120 题，**只截断书系并入的题**（它们排在列表末尾），因此既有「深入浅出 531 / 罗勇军 607 / 刘汝佳 60 / CF 507」题单一条未少 —— 核心题单 1719 题保持不变。

界面侧：题目卡片新增 `data-problem-source`，节点页题目区多出「全部来源」下拉（8 个来源可按需筛选），书系题目自带洛谷算法标签芯片可直接跳标签页。

### 3. 顺带修掉的真 bug：`A*` 标签会写坏构建

并入书系题单后构建直接失败：`ENOENT: no such file or directory, open 'site/data/tags/A*.json'`。

根因是**`encodeURIComponent` 按 RFC 3986 保留 `*`**（保留集是 `! ~ * ' ( )`），而 `*` 在 Windows 文件系统上是通配符。标签分片（`data/tags/<tag>.json`）与标签页目录（`tags/<tag>/index.html`）此前都直接用 `encodeURIComponent(tag)` 拼路径，于是标签 `A*`、`IDA*`（`lib/luogu-tag-map.mjs` 里 `A* 算法` 的规范名）必然踩雷 —— 这是**一直在仓库里、只是此前没有任何日志用到这两个标签**的潜伏缺陷，被新并入的题单激活。

修法是新增 `lib/tag-index.mjs#tagStorageKey()`：只保留字母/数字/连字符/下划线/点，其余一律百分号编码，构建端（分片写入、标签页目录、路由 index）与浏览器端（`lib/data.mjs` 取分片、`tagHref()` 生成链接）共用同一份实现。链接也必须换：`encodeURIComponent("A*")` 仍是 `A*`，而浏览器访问 `/tags/A*/` 时路径会被规范化成字面星号，与 `%2A` 落盘的目录对不上，必然 404。`scripts/preview-ui.mjs` 的静态服务器同步改为"解码路径找不到时回退原样路径"，与静态托管对百分号目录的处理一致。

**验证**：`test/tag-storage-key.test.mjs`（6 项，含"编码结果不含任何文件系统不安全字符"与"tagHref 必须与 encodeURIComponent 不同"）、`test/generate-seo.test.mjs` 新增"含特殊字符的标签必须存在分片与标签页"回归断言；真实浏览器确认 `/tags/A%2A/` 正常渲染。
...
| --- | --- |
| 语法检查 | 90 个源文件通过 |
| 单元/回归测试 | 460 + 76 = **536 项全绿**（本轮新增 22 项） |
| 构建 | `node scripts/generate-data.js` 通过，193 条日志，书系题嵌入 39 个节点 |
| 浏览器复核 | `artifacts/verify-v21.mjs` 10 项断言全过（来源下拉、按来源筛选、星号标签页、难度徽标配色、总览渲染） |
| 对比度扫描 | `artifacts/contrast-audit.mjs` 严重级低对比条目 2 → **0** |

这一轮印证了阶段六那句判断的另一种形态：**"黑底黑字"同样不是漏写，而是同一份配色被两处声明、后一处把前一处整个抹掉**。和阶段六的"训练日口径分裂"一样，症状在页面上，病根在"同一件事有两个来源"。所以三处修复都指向同一个方向 —— 把口径收成一个可被测试调用的模块，而不是散在样式表、模板和构建脚本里的重复声明。
