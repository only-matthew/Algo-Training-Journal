...
依据：`git log` 全部 358 个提交（2026-07-23 → 2026-09-27）与 `docs/` 全部文档。凡引用提交均给出短哈希，凡引用文档均给出文件与日期。

---

## 概览

下表是**本文档所在提交的快照**；此后新增的提交不再回填，引用时请以 `git rev-list --count HEAD` 为准。
...
| 其中工程提交 | 141 |
| 跟踪文件数 | 22（首提交）→ 939（09-25 清理后）→ 969（本提交） |
| 工程提交分布 | 7 月 31 · 8 月 45 · 9 月 65 |
...
| 作者 | 工程提交 | 日志提交 |
| --- | --- | --- |
| only-matthew（廖夏，维护者） | **132**（另有 4 个用了另一种名字拼写） | 98 |
| wzzzzhhhhh（王梓豪） | 3 | 63 |
| Yiming Guo（郭一鸣） | 2 | 56 |

即：**这是一个近乎单人维护的项目**，另外两名队员主要通过网页表单贡献训练数据，偶尔提交少量修复。358 个提交里有 217 个（61%）是**应用自己写出来的数据**，不是人写的代码 —— 这正是"仓库即数据库"架构在版本史上的直接体现。
...
![ACM 训练日志项目发展时间轴总览](assets/project-history/timeline-overview.png)

*总览：六个阶段与三个量化指标。图中"354 次提交"是该图生成时的快照，本页随后又落了若干提交，最新数字以上方表格为准。*

---

## 阶段一：MVP 与数据布局定型（07-23 → 07-24）

![系列一：基础成型](assets/project-history/timeline-01-foundation.png)

*系列一「基础成型」（07-23 → 08-01）覆盖下面的阶段一与阶段二：首个可用版本 → 数据布局重构 → 错题本/标签/OAuth → 题号与原题链接 → 第一次工程化重构。*
...
![系列二：平台扩张与 v2 试验](assets/project-history/timeline-02-expansion-v2.png)

*系列二「平台扩张与 v2 试验」（08-05 → 09-11）覆盖下面的阶段三与阶段四：三平台导入 → 知识树与 QQ 机器人 → 掌握度闭环与首次系统审查 → v2 工作台上线 → 公开入口下线。*
...
![系列三：可靠性、规模化与审计](assets/project-history/timeline-03-reliability-audit.png)

*系列三「可靠性、规模化与审计」（09-13 → 09-26）覆盖下面的阶段五与阶段六：活力与区间训练 → 条件写入与题面 PDF → 学习状态拆分 → 图片归档与 AtCoder 三级降级 → 分片构建（目标 10000 条）→ 22 项外部审计。*
