...
export function buildMDContent(log) {
  ...
  const tableRows = [
    ...
    `| 本次失误 | ${state.isMistake ? "有" : "无"} |`,
    `| 掌握自评 | ${markdownCell(MASTERY_LABELS[state.masteryStatus])} |`,
    `| 复习安排 | ${markdownCell(REVIEW_LABELS[state.reviewStatus])} |`,
  ].join("\n");
  ...
  if (log.statementUrl) parts.push("## 原题附件", "", "PDF 原题附件已随站点发布。", "");
  if (log.aiAnalysis?.result) parts.push("## 题目分析", "", String(log.aiAnalysis.result.summary || "").trim(), "", String(log.aiAnalysis.result.analysis?.approach || "").trim(), "");
...
const LATEX_SYMBOLS = {
  ...
  // 洛谷难度用实心/空心星标表示（如 "★ 1200"），正文字体没有这两个字形。
  "★": "\\bigstar", "☆": "\\star",
  "⋯": "\\cdots", "…": "\\dots", "°": "^\\circ", "′": "'", "″": "''",
  "−": "-", "–": "--", "—": "---", "‘": "`", "’": "'", "“": "``", "”": "''",
...
export function escapeLatexText(value) {
  return String(value ?? "")
    .replace(/\r\n?/g, "\n")
    // Give long plain URLs legal breakpoints without changing their visible text.
...
function escapeMathTextArgument(value) {
  return String(value ?? "")
    .replace(LATEX_SPECIALS_RE, (ch) => LATEX_ESCAPES[ch])
    .replace(LATEX_SYMBOLS_RE, (ch) => {
      if (ch === "∗") return "*";
      return `\\ensuremath{${LATEX_SYMBOLS[ch]}}`;
    });
}

function renderMath(formula) {
  const textArgs = [];
  const protectedFormula = String(formula ?? "").replace(/\\text\{([^{}]*)\}/g, (_, body) => {
    const index = textArgs.push(escapeMathTextArgument(body)) - 1;
    return `\u0002${index}\u0003`;
  });
  return protectedFormula
    // Some contest statements emit MathJax aliases that are not defined by amsmath.
    .replace(/\\gt\b/g, ">")
    .replace(/\\lt\b/g, "<")
    .replace(LATEX_SYMBOLS_RE, (ch) => LATEX_SYMBOLS[ch])
    .replace(/,/g, ",\\allowbreak{}")
    .replace(/\u0002(\d+)\u0003/g, (_, index) => `\\text{${textArgs[Number(index)]}}`);
}
...
function buildLatexContent(log) {
  ...
  const lines = [
    ...
    `本次失误 & ${state.isMistake ? "有" : "无"} \\\\ \\hline`,
    `掌握自评 & ${escapeLatexText(MASTERY_LABELS[state.masteryStatus]).trim()} \\\\ \\hline`,
    `复习安排 & ${escapeLatexText(REVIEW_LABELS[state.reviewStatus]).trim()} \\\\ \\hline`,
    "\\end{tabular}",
    "",
  ];
