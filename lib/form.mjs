...
export function createProblemRow(index) {
  ...
  div.querySelector(".btn-fetch-statement").addEventListener("click", async () => {
    ...
    try { const result = await fetchProblemStatement(problemNumber); if (result.status !== "ok") { status.textContent = `题面暂不可用：${STATEMENT_REASON_TEXT[result.reason] || result.reason}`; return; } const description = div.querySelector(".problem-description"); if (description.value.trim()) { const panel = div.querySelector(".problem-enrichment"); panel.hidden = false; panel.querySelector(".analysis-preview").hidden = false; panel.querySelector(".analysis-preview").textContent = `已抓取题面，尚未覆盖现有描述：\n\n${result.description}`; div.dataset.statementPreview = JSON.stringify(result); status.textContent = result.source?.kind === "luogu-mirror" ? "已获取题面（来自洛谷镜像，可能是中文翻译），请核对预览后再手动替换描述。" : "已获取题面，请核对预览后再手动替换描述。"; return; } description.value = result.description; const existing = JSON.parse(div.dataset.enrichment || "{}"); div.dataset.enrichment = JSON.stringify({ ...existing, statementSource: result.source }); const notes = [result.source?.kind === "luogu-mirror" ? "题面取自洛谷镜像，可能是中文翻译，建议对照原题核对" : "", result.warnings?.includes("external-images") ? "含外链图片，建议同时上传 PDF" : ""].filter(Boolean); status.textContent = notes.length ? `题面已填入；${notes.join("；")}。` : "题面已填入。"; markFormEdited(); } catch (error) { status.textContent = `抓取失败：${error.message}`; }
  });
