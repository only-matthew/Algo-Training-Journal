import { createProblemId, LOG_LIMITS, REVIEW_STATUSES, OUTCOMES, logInputBytes, validateLogInput } from "./log-schema.mjs";
import { apiRequest, deleteDateLog, loadDateLog, saveDateLog, saveDateLogV2, statementUrl, importCodeforces, importLuogu, importAtCoder, fetchProblemStatement } from "./journal-api.js";
...
export function collectProblems() {
  ...
  for (const block of blocks) {
    ...
    problems.push({
      ...
      code: f.code.trim(),
      ...Object.fromEntries(Object.entries(f).filter(([key]) => ["statementAttachment", "statementSource", "metadataSources", "aiAnalysis"].includes(key))),
    });
  }
  return problems;
}
...
async function saveWithAttachments(date, problems, interval, revision) {
  const log = {
    schemaVersion: 4,
    problems: problems.map(({ statementAttachment, ...problem }) => problem),
    ...interval,
  };
  ...
  const result = await saveDateLogV2(date, { log, expectedVersion: revision, attachmentChanges, attachments, operationId });
...
export function createProblemRow(index) {
  ...
  div.innerHTML = `
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        <label for="prob-${index}-difficulty">难度（Codeforces Rating）</label>
    ...
    <div class="form-group">
      ...
      <section class="problem-enrichment" hidden>
        ...
        <textarea class="analysis-json form-input" rows="8" maxlength="65536" placeholder="粘贴 DeepSeek 返回的完整 JSON"></textarea>
  ...
  div.querySelector(".btn-fetch-statement").addEventListener("click", async () => {
    ...
    try { const result = await fetchProblemStatement(problemNumber); if (result.status !== "ok") { status.textContent = `题面暂不可用：${result.reason}`; return; } const description = div.querySelector(".problem-description"); if (description.value.trim()) { const panel = div.querySelector(".problem-enrichment"); panel.hidden = false; panel.querySelector(".analysis-preview").hidden = false; panel.querySelector(".analysis-preview").textContent = `已抓取题面，尚未覆盖现有描述：\n\n${result.description}`; div.dataset.statementPreview = JSON.stringify(result); status.textContent = "已获取题面，请核对预览后再手动替换描述。"; return; } description.value = result.description; const existing = JSON.parse(div.dataset.enrichment || "{}"); div.dataset.enrichment = JSON.stringify({ ...existing, statementSource: result.source }); status.textContent = result.warnings?.length ? "题面已填入；含外链图片，建议同时上传 PDF。" : "题面已填入。"; markFormEdited(); } catch (error) { status.textContent = `抓取失败：${error.message}`; }
  });
...
export function populateProblems(parsed) {
  ...
  parsed.forEach((p, i) => {
    ...
    row.querySelector(".problem-outcome").value = p.outcome || "";
...
async function restorePendingAttachments() {
  ...
  for (const problemId of orphans) await attachmentStore.remove(memberId, date, problemId);
  if (restored) refreshStatementStates();

  const msgEl = document.getElementById("submit-msg");
  if (!msgEl) return;
  if (restored) msgEl.textContent = "已恢复该日期尚未上传的题面 PDF，保存后会归档到仓库。";
  else if (orphans.length) msgEl.textContent = "之前选择的题面 PDF 已无法对应到当前题目，请重新选择。";
}
...
export async function handleSubmit() {
  ...
  try {
    ...
    try {
      ...
      const revision = await ensureFormRevision(date);
      // 只要本次涉及附件（新增/替换/移除），就必须走 v2：旧接口没有上传 PDF 字节的能力。
      const result = pendingAttachments.size
