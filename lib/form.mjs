...
// ── 草稿持久化（localStorage）：误关模态框 / 刷新页面后仍可恢复未提交内容 ──
const DRAFTS_KEY = "journal-drafts-v1";
const DRAFT_MAX_BYTES = 1.2 * 1024 * 1024; // 单日上限 1.5MB，留出余量；超限仅保留内存草稿
...
function persistDraft(date, snapshot) {
  try {
    if (!snapshot || !Array.isArray(snapshot.problems)) return;
    if (JSON.stringify(snapshot).length > DRAFT_MAX_BYTES) return;
    const drafts = loadStoredDrafts();
    drafts[date] = snapshot;
...
function removeStoredDraft(date) {
  try {
    const drafts = loadStoredDrafts();
...
export function updateSubmissionSummary() {
  ...
  // 当前日期的未提交内容同步到 localStorage（空表单则清理该日期草稿）
  if (activeFormDate) {
    ...
    else removeStoredDraft(activeFormDate);
  }
}
...
export async function onDateChange() {
  ...
  if (activeFormDate && activeFormInitialized) {
    ...
    dateDrafts.set(activeFormDate, snapshot);
  ...
  // localStorage 草稿优先于服务端：用户在误关/刷新前输入的内容不应被服务端数据覆盖
  const stored = loadStoredDrafts()[date];
  if (stored && Array.isArray(stored.problems)) {
    dateDrafts.set(date, stored);
    populateProblems(stored.problems);
    setDateFormState(date, stored.exists);
    activeFormInitialized = true;
    setDateLoadState("ready");
