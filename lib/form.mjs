...
export async function runImport() {
  ...
  try {
    ...
    status.textContent = importResults.length
      ? `共找到 ${importResults.length} 题，勾选后点击「添加到表单」。${importPlatform === "luogu" ? "（洛谷不提供标签接口，标签需在表单中手动补充）" : ""}`
      : "没有找到可导入的题目。";
...
function renderImportList() {
  ...
  importResults.forEach((p, i) => {
    ...
    const diff = p.platform === "Codeforces" && p.rating
      ? ` · ${cfRatingToDifficulty(p.rating)}`
