...
export function closeModal(force = false) {
  if (!force && activeFormDate) persistDraft(activeFormDate, { problems: captureProblemDrafts(), exists: activeFormExists });
...
function createProblemRow(index) {
  ...
  div.innerHTML = `
    ...
    <details class="journal-disclosure journal-statement-tools">
      <summary>题面与附件 <span>抓取题面、AI 辅助与 PDF 归档</span></summary>
...
export function updateSubmissionSummary() {
  ...
  if (activeFormDate && activeFormLoadState === "ready" && activeFormInitialized) {
...
export function saveDraftNow() {
  ...
  const result = persistDraft(date, { problems: captureProblemDrafts(), exists: activeFormExists });
  if (!result.savedAt) {
