...
function persistDraft(date, snapshot) {
  ...
  const previous = dateDrafts.get(date);
  const baseRevision = Object.hasOwn(snapshot || {}, "baseRevision") ? snapshot.baseRevision
    : previous && Object.hasOwn(previous, "baseRevision") ? previous.baseRevision
      : activeFormRevision ?? null;
  const complete = { ...snapshot, baseRevision };
  const memoryVersion = rememberDraft(date, complete);
  if (!memberId || !snapshot || !Array.isArray(snapshot.problems)) {
    return { savedAt: null, memoryVersion };
  }
  const result = draftStore.save({ memberId, date, baseRevision, problems: snapshot.problems, exists: snapshot.exists, interval: snapshot.interval });
...
async function fetchStatementForBlock(div) {
  ...
  try {
    ...
    if (result.status !== "ok") {
      const blockedNote = result.reason === "blocked"
        ? `${platform} 官方页在服务端被拦截，且洛谷没有这道题的镜像；如果浏览器能打开原题，可用下面的「从官方题目页导入」把题面取回来；`
        : "";
 ...
 * 「抓取当前题面」小书签。
 *
 * 它在 Codeforces / AtCoder 题目页里被点击：把整页 outerHTML 复制到剪贴板，
 * 用户回到这里粘贴即可。无痕窗口也能用，正好绕过服务器出口被上游拦截的问题。
 * 站点 CSP 不允许在本站执行它，所以只能拖到书签栏使用——这正是它绕过跨域限制的原因。
 */
const STATEMENT_BOOKMARKLET = "javascript:(()=>{try{const h=document.documentElement.outerHTML;const t=document.createElement('textarea');t.value=h;t.setAttribute('readonly','');t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();const ok=document.execCommand('copy');t.remove();alert(ok?('已复制当前题目页源码（'+Math.round(h.length/1024)+' KB）。回到训练日志，粘贴到「从官方题目页导入」里再点「解析并填入」。'):'复制失败，请改用 Ctrl+U 打开源码后 Ctrl+A、Ctrl+C。');}catch(e){alert('复制失败：'+e.message+'。请改用 Ctrl+U 打开源码后 Ctrl+A、Ctrl+C。');}})()";

function validateAnalysisForBlock(div) {
  ...
  div.querySelector(".btn-apply-analysis").disabled = activeFormLoadState !== "ready";
  return result;
}
...
function createProblemRow(index) {
  ...
  div.innerHTML = `
    ...
    <div class="form-group">
      ...
      <details class="journal-disclosure statement-import">
        <summary>从官方题目页导入（服务器抓不到时用）</summary>
        <div class="journal-disclosure-body">
          <p class="statement-import-help">Codeforces 或 AtCoder 有时会拦截服务器，但你的浏览器仍能打开原题。把下面的小书签拖到书签栏，在题目页（包括无痕窗口）点一下复制源码，再回来粘贴：</p>
          <p class="statement-import-actions"><a class="btn-bookmarklet" href="${STATEMENT_BOOKMARKLET}" title="拖到书签栏；在 Codeforces 或 AtCoder 题目页点击即可复制源码">抓取当前题面</a><button type="button" class="btn-copy-bookmarklet">复制小书签代码</button><span class="statement-import-fallback">也可以在题目页按 Ctrl+U 打开源码后 Ctrl+A、Ctrl+C。</span></p>
          <textarea class="form-input statement-html" rows="3" spellcheck="false" placeholder="在这里粘贴 Codeforces / AtCoder 题目页源码（Ctrl+V），然后点「解析并填入」"></textarea>
  ...
  // 小书签路径：CF / AtCoder 可能拦截服务器出口，浏览器又受 CORS 限制；用户在能打开
  // 原题的浏览器（包括无痕窗口）点一下书签，把页面源码复制回来解析。
  ...
  div.querySelector(".btn-copy-bookmarklet").addEventListener("click", async () => {
    const status = div.querySelector(".summarize-status");
    try {
      await navigator.clipboard.writeText(STATEMENT_BOOKMARKLET);
      status.textContent = "小书签代码已复制：到浏览器书签管理器新建书签，把代码粘到「网址」里，然后打开 Codeforces 或 AtCoder 题目页点它。";
    } catch {
  ...
  div.querySelector(".btn-parse-statement-html").addEventListener("click", async () => {
    ...
    if (!html) { status.textContent = "请先粘贴当前题目页源码（点上面小书签，或 Ctrl+U 全选复制）。"; return; }
  ...
  analysisInput.addEventListener("input", () => {
    ...
    analysisApplyButton.disabled = activeFormLoadState !== "ready" || !analysisInput.value.trim();
  });
  div.querySelector(".btn-parse-analysis").addEventListener("click", () => {
    const status = div.querySelector(".summarize-status");
    try {
      validateAnalysisForBlock(div);
  ...
  analysisApplyButton.addEventListener("click", () => {
    const status = div.querySelector(".summarize-status");
    try {
      // “预览 JSON”是可选检查步骤；用户粘贴后直接点应用时，在这里完成同一套严格验证。
      const result = cachedAnalysisResult(div) || validateAnalysisForBlock(div);
...
function renderUndoButton() {
  const button = document.getElementById("btn-undo-remove");
  if (!button) return;
  button.hidden = removedProblems.length === 0;
  button.disabled = activeFormLoadState !== "ready";
}

export function removeProblem(block) {
  const list = document.getElementById("problem-list");
  if (!block || block.parentElement !== list || activeFormLoadState !== "ready") return;
  removedProblems.push({ block, next: block.nextSibling, date: activeFormDate });
  block.remove();
  reindexProblemBlocks();
  markFormEdited();
  renderUndoButton();
}

export function undoRemovedProblem() {
  const removed = removedProblems.pop();
  if (!removed || removed.date !== activeFormDate) return;
  const list = document.getElementById("problem-list");
  if (removed.next?.parentElement === list) list.insertBefore(removed.block, removed.next);
  else list.appendChild(removed.block);
  reindexProblemBlocks();
  markFormEdited();
  renderUndoButton();
  removed.block.querySelector(".problem-name")?.focus();
}
...
export function setDateLoadState(state) {
  ...
  if (!blocked) for (const button of document.querySelectorAll("#problem-list .btn-apply-analysis")) {
    const block = button.closest(".problem-block");
    button.disabled = !block?.querySelector(".analysis-json")?.value.trim();
  }
...
export async function onDateChange({ syncUrl = true } = {}) {
  ...
  const draft = dateDrafts.get(date) || (stored.status === "found" ? {
    ...
    baseRevision: stored.draft.baseRevision,
  } : null);
  ...
  try {
    ...
    activeServerProblems = serverProblems.map((p) => ({ id: p.id, name: p.name, problemNumber: p.problemNumber }));
    const draftProblems = draft?.problems || [];
    const staleOverlap = Boolean(draft && draft.baseRevision !== (loaded.revision ?? null)
      && draftProblems.some((p) => serverProblems.some((saved) => saved.id === p.id)));
    ...
    if (staleOverlap) {
      activeFormConflictRevision = loaded.revision ?? null;
      msgEl.textContent += "同一道题的服务器版本已变化，保存前会要求确认是否覆盖。";
    }
    if (draft) {
      rememberDraft(date, { problems: captureProblemDrafts(), exists, interval: readTrainingInterval(), baseRevision: draft.baseRevision }, stored.status === "found" ? stored.draft.savedAt : undefined);
    }
    activeFormInitialized = true;
...
function removedServerProblems(problems) {
  const ids = new Set(problems.map((problem) => problem.id));
  return activeServerProblems.filter((problem) => !ids.has(problem.id));
}

function renderSavePreview() {
  const preview = document.getElementById("submission-save-preview");
  if (!preview) return;
  if (activeFormLoadState !== "ready") {
    preview.textContent = "正在确认这一天的记录…";
    preview.classList.remove("is-warning");
    return;
  }
  const problems = collectProblems();
  const removed = removedServerProblems(problems);
  preview.textContent = `将保存 ${problems.length} 道题；服务器已有 ${activeServerProblems.length} 道${removed.length ? `，其中 ${removed.length} 道将被移除` : ""}`;
  preview.classList.toggle("is-warning", removed.length > 0);
}
...
export async function handleSubmit() {
  ...
  try {
    ...
    try {
      validateLogInput(
        { problems, ...interval },
        { recordDate: date, today: toUtc8(new Date().toISOString()).slice(0, 10) },
      );
    ...
    const removed = removedServerProblems(problems);
    if (removed.length) {
      const names = removed.map((p) => p.problemNumber || p.name || p.id).join("、");
      if (!confirm(`这次将保存 ${problems.length} 道题，但服务器原有的 ${removed.length} 道题（${names}）会被移除。确认继续吗？`)) {
        document.getElementById("submit-msg").textContent = "已取消保存；原有题目未被覆盖。";
        return;
      }
    }
    ...
    try {
      ...
      activeFormDirty = false;
      activeServerProblems = problems.map((p) => ({ id: p.id, name: p.name, problemNumber: p.problemNumber }));
      removedProblems.length = 0;
      renderUndoButton();
      renderSavePreview();
