...
import { apiRequest, deleteDateLog, loadDateLog, saveDateLog, importCodeforces, importLuogu } from "./journal-api.js";
...
// ── 快速导入（Codeforces AC 记录 / 洛谷题号补全）──

let importPlatform = "codeforces";
let importResults = [];
let importChecked = new Set();

function cfRatingToDifficulty(rating) {
...
export function openImportPanel(platform) {
  ...
  // 预填队员预置的 Codeforces 用户名（可在输入框内修改）
...
export async function runImport() {
  ...
  try {
    ...
    status.textContent = importResults.length
      ? `共找到 ${importResults.length} 题，勾选后点击「添加到表单」。`
      : "没有找到可导入的题目。";
...
function renderImportList() {
  ...
  importResults.forEach((p, i) => {
    ...
    const diff = p.platform === "Codeforces" && p.rating ? ` · ${cfRatingToDifficulty(p.rating)}` : "";
    span.textContent = `${p.platform} ${p.problemNumber || ""} · ${p.name}${diff}`;
...
function fillProblemRow(row, p) {
  ...
  row.querySelector(".problem-number").value = p.problemNumber || "";
  if (p.platform === "Codeforces" && p.rating) {
    row.querySelector(".problem-difficulty").value = cfRatingToDifficulty(p.rating);
  }
