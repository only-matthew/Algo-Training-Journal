...
export function createProblemRow(index) {
  ...
  div.innerHTML = `
    ...
    <div class="form-group">
      <div class="form-label-row">
        ...
        <span><button type="button" class="btn-fetch-statement" title="按当前的平台与题号抓取官方题面（Codeforces / AtCoder）">抓取题面</button> <button type="button" class="btn-ai-enrich">DeepSeek 提示词</button><button type="button" class="btn-summarize" title="强烈建议用 AI 概括题目描述，减少篇幅">AI 概括</button></span>
      </div>
  ...
  div.querySelector(".btn-fetch-statement").addEventListener("click", async () => {
    const status = div.querySelector(".summarize-status"); const platform = div.querySelector(".problem-platform").value; const problemNumber = div.querySelector(".problem-number").value.trim();
    if (!STATEMENT_PLATFORMS.has(platform) || !problemNumber) { status.textContent = "请先填写 Codeforces 或 AtCoder 题号。"; return; }
    if (platform === PLATFORMS.ATCODER && !ATCODER_NUMBER.test(problemNumber)) { status.textContent = "AtCoder 题号要写成 abc381_a 这样的任务 ID。"; return; }
    status.textContent = "抓取题面中…";
    try {
      const result = await fetchProblemStatement(platform, problemNumber);
      if (result.status !== "ok") {
        // AtCoder 官方页在服务端整体 403（机房出口被拦），走到这条失败分支说明洛谷也没有
        // 这道题的 AT_ 镜像——不把话说清楚，用户会以为是自己的题号或网络有问题。
        const blockedNote = platform === PLATFORMS.ATCODER && result.reason === "blocked" ? "AtCoder 官方页在服务端被拦截，且洛谷没有这道题的镜像；" : "";
      ...
      if (description.value.trim()) {
        ...
        status.textContent = [`已获取题面（尚未覆盖现有描述），请核对预览后再手动替换描述`, sourceNote, languageNote, imageNote, externalNote].filter(Boolean).join("；") + "。";
        return;
      }
      ...
      const notes = [sourceNote, languageNote, imageNote, externalNote].filter(Boolean);
