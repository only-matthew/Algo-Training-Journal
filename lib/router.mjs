...
export function navigateTo(path) {
  ...
  if (`${window.location.pathname}${window.location.search}${window.location.hash}` === `${next.pathname}${next.search}${next.hash}`) return;
...
export function initPageNavigation() {
  ...
  function showRequestedPage() {
    ...
    if (route === "submit") pageId = "submission-page";
    document.body?.classList?.toggle("submission-route", pageId === "submission-page");
    ...
    if (pageId === "submission-page" && typeof window.journalSubmissionRouteRenderer === "function") {
      window.journalSubmissionRouteRenderer();
    }
