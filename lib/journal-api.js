...
export async function loadSession({ timeoutMs = SESSION_TIMEOUT_MS } = {}) {
  const data = await apiRequest("/api/session", { signal: AbortSignal.timeout(timeoutMs) });
...
export function patchRecordReview(date, recordId, patch) {
  return apiRequest(`/api/v2/me/logs/dates/${encodeURIComponent(date)}/records/${encodeURIComponent(recordId)}`, {
    method: "PATCH",
...
// 浏览器小书签抓回的 Codeforces / AtCoder 官方页源码：服务端不再请求上游，直接解析。
