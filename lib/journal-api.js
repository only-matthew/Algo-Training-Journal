...
/**
 * v4 附件保存：multipart 提交 payload + 每个待上传 PDF 一个分区。
 *
 * 旧 JSON 接口没有上传字节的能力，因此只要涉及新增/替换 PDF 就必须走这里。
 ...
 * @param {Map<string,{blob: Blob, fileName: string}>} [options.attachments] partName -> 文件
 * @param {string} options.operationId    幂等键（UUID）
 */
export function saveDateLogV2(date, { log, expectedVersion, attachmentChanges, attachments, operationId }) {
...
export function fetchProblemStatement(problemNumber, sourceUrl) { return apiRequest("/api/problem-statement", { method: "POST", body: JSON.stringify({ platform: "Codeforces", problemNumber, ...(sourceUrl ? { sourceUrl } : {}) }) }); }
