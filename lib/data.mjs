...
export async function ensureFullJournal(force = false) {
  ...
  fullJournalPromise = ensureJournalManifest(force)
    .then((index) => loadShardList(index.months, force))
    .then((logs) => journalWithLogs(logs, force))
...
export async function ensureJournalManifest(force = false) {
  ...
  manifest = await fetchJson("data/manifest.json", force);
  return manifest;
}
...
async function journalWithLogs(logs, force = false) {
  const overview = await ensureOverviewJournal(force);
  return { ...overview, logs };
}

export async function ensureAnalysisJournal(start, end, force = false) {
  const index = await ensureJournalManifest(force);
  const firstMonth = (start || "0000-00").slice(0, 7);
  const lastMonth = (end || "9999-99").slice(0, 7);
  const entries = index.months.filter((entry) => entry.id >= firstMonth && entry.id <= lastMonth);
  return journalWithLogs(await loadShardList(entries, force), force);
}

export async function ensureMemberJournal(member, force = false) {
  const index = await ensureJournalManifest(force);
  return journalWithLogs(await loadShardList(index.members?.[member]?.years || [], force), force);
}

export async function ensureReviewJournal(force = false) {
  const index = await ensureJournalManifest(force);
  return journalWithLogs(await loadShardList([index.review], force), force);
}
...
export function loadTagDetail(tag, force = false) {
  return fetchJson(`data/tags/${encodeURIComponent(tag)}.json`, force);
}
