...
export function trainingCardHtml(log) {
  ...
  return `<div class="record-title-row"><h3 class="record-title"><a href="${href}">${log.problemNumber && !String(log.problem).includes(log.problemNumber) ? `<span class="problem-number">${escapeHtml(log.problemNumber)}</span> ` : ""}${escapeHtml(log.problem)}</a></h3><span class="difficulty-badge ${tone}">${escapeHtml(difficulty)}</span>${vitality}</div>
    ${summary ? `<p class="record-summary">${escapeHtml(summary)}</p>` : ""}
    <div class="record-badges">${tags.slice(0, 4).map(tag => `<a class="tag-chip" href="/tags/${encodeURIComponent(tag)}/">${escapeHtml(tag)}</a>`).join("")}${tags.length > 4 ? `<a class="tag-chip" href="${href}" aria-label="查看其余 ${tags.length - 4} 个标签">+${tags.length - 4}</a>` : ""}${state.isMistake ? '<span class="review-chip todo">本次有失误</span>' : ""}${state.masteryStatus !== "unknown" ? `<span class="review-chip mastered">${escapeHtml(MASTERY_LABELS[state.masteryStatus])}</span>` : ""}${state.reviewStatus !== "none" ? `<span class="review-chip ${state.reviewStatus === "todo" ? "todo" : "mastered"}">${escapeHtml(REVIEW_LABELS[state.reviewStatus])}</span>` : ""}</div>
