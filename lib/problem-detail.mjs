...
export function problemDetailHtml(log, { sourceUrl = "", memberHref = "" } = {}) {
  ...
  return `<header class="page-hero problem-hero"><div class="hero-art" aria-hidden="true"></div>
    ...
    </div><aside class="problem-aside"><section class="detail-panel"><div class="section-heading"><h2>${icon("file")}题目信息</h2><div data-problem-edit hidden></div></div><dl class="problem-facts"><dt>题目编号</dt><dd>${escapeHtml(log.problemNumber || "未填写")}</dd><dt>来源平台</dt><dd>${escapeHtml(log.platform)}</dd><dt>难度</dt><dd>${escapeHtml(log.difficulty || "未标注")}</dd><dt>记录成员</dt><dd><a href="${escapeHtml(memberHref)}">${escapeHtml(log.member)}</a></dd><dt>训练日期</dt><dd>${escapeHtml(log.date)}</dd><dt>复习状态</dt><dd>${log.reviewStatus === "todo" ? "待复习" : log.reviewStatus === "mastered" ? "已掌握" : "未加入复习"}</dd></dl><div class="problem-review-actions" data-problem-review role="group" aria-label="复习操作" hidden></div></section>
