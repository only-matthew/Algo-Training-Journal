import { PLATFORMS, toDateString } from "./constants.mjs";
import { REVIEW_LABELS, REVIEW_STATUSES, normalizeLearningState, isReviewPlanned, isReviewTodo } from "./learning-state.mjs";
import { escapeHtml } from "./escape-html.mjs";
import {
  BATCH_MAX_MD, BATCH_MAX_PDF, PRINT_CSS, buildBatchLatexDocument, buildMDContent,
  buildSingleLatexDocument, mdToPrintableHTML, safeFilename,
} from "./export-content.mjs";
import { originalProblemUrl } from "./problem-links.mjs";
...
import { loadProblemDetail, loadRoadmapNode, loadTagDetail, forceProblemDetailRefresh, clearForceRefresh, problemDetailSequence, nextProblemDetailSequence } from "./data.mjs";
import { currentUser } from "./auth.mjs";
import { journalScopeForRoute } from "./journal-scope.mjs";
import { icon } from "./icons.mjs";
import { patchRecordReview } from "./journal-api.js";
import { dueDateInDays } from "./review-utils.mjs";
import { ratingLabel, ratingOptions } from "./rating.mjs";
import { mergeTrainingDates } from "./training-interval.mjs";
import { vitalityLevel } from "./vitality.mjs";
import { memberVitalityDetailsHtml } from "./member-vitality.mjs";
import { renderVitalityChart } from "./vitality-chart.mjs";
export { renderVitalityChart } from "./vitality-chart.mjs";

// 难度统一按 CF Rating 展示；历史记录没有 Rating 时回退到原难度标签。
function difficultyText(log) {
  return ratingLabel(log?.difficultyRating, String(log?.difficulty || "未标注"));
}

import { trainingCardHtml } from "./ui.mjs";
...
export async function renderEnhancements(root) {
...
function logCardHtml(log) { return trainingCardHtml(log); }
...
// 活力口径的热力图分档在 lib/vitality.mjs 里（纯函数，便于单测）
export function renderHeatmap(root, counts, member, members, byMember, values, valuesByMember) {
  const end = new Date();
  const calendarLabel = document.querySelector(`[data-calendar-label="${root.id}"]`);
  if (calendarLabel) calendarLabel.textContent = `${end.getFullYear()} 年 ${end.getMonth() + 1} 月`;
  const start = new Date(end);
  // Each column is a Monday–Sunday week; leave future days in the last week blank.
  start.setDate(end.getDate() - ((end.getDay() + 6) % 7) - 29 * 7);

  root.innerHTML = "";

  const useVitality = Boolean(values);
  // 可见图例：说明色深按活力指数而非题数（做 1 道提高题就能到深色）
  const legendEl = root.closest?.(".calendar-panel")?.querySelector(".heatmap-legend");
  if (legendEl) {
    legendEl.title = useVitality ? "按当日活力指数上色" : "按当日训练题数上色";
    legendEl.setAttribute("aria-label", useVitality
      ? "热力图色阶：颜色越深表示当日活力指数越高"
      : "热力图色阶：颜色越深表示当日训练题数越多");
  }
  const legend = document.createElement("div");
  legend.className = "sr-only";
  legend.textContent = useVitality
    ? "热力图：颜色越深表示当日活力指数越高。"
    : "热力图：颜色越深表示当日训练题数越多。白色=0题，浅绿=1题，中绿=2题，深绿=3-4题，墨绿=5题以上。";
  root.appendChild(legend);

  // 容器作为单一可聚焦元素，避免每个活跃日成为独立 tab 停靠点（最多 365 个）
  root.tabIndex = 0;
  root.setAttribute("role", "group");
  root.setAttribute("aria-label", useVitality
    ? "近30周训练热力图：每个单元格表示一天的活力指数，可聚焦后浏览"
    : "近30周训练热力图：每列为周一至周日，每个单元格表示一天的训练题数，可聚焦后浏览");
  ...
  for (let i = 0; i < 210; i += 1) {
    ...
    if (date > end) {
      ...
      placeholder.className = "cell cell-future";
    ...
    const value = useVitality ? (values[dateText] || 0) : 0;

    let tooltip = useVitality
      ? `${dateText} · 活力 ${value.toFixed(2)}${count ? ` · ${count} 题` : ""}`
      : `${dateText} · ${count} 题`;
    if (member === "all" && count > 0) {
      const contributions = members
        .map((m) => ({
          name: m,
          cnt: (counts && byMember[m] && byMember[m][dateText]) || 0,
          val: (valuesByMember && valuesByMember[m] && valuesByMember[m][dateText]) || 0,
        }))
        .filter((c) => c.cnt > 0 || c.val > 0)
        .sort((a, b) => b.val - a.val || b.cnt - a.cnt || a.name.localeCompare(b.name, "zh-CN"));
      const lines = contributions.map((c) => (useVitality
        ? `${c.name}: 活力 ${c.val.toFixed(2)}（${c.cnt} 题）`
        : `${c.name}: ${c.cnt} 题`));
      tooltip = [tooltip, ...lines].join("\n");
    }

    const cell = document.createElement("span");
    cell.className = `cell lv${useVitality ? vitalityLevel(value) : levelOf(count)}`;
    cell.title = tooltip;
    cell.setAttribute("aria-label", useVitality
      ? `${dateText}：活力 ${value.toFixed(2)}`
      : `${dateText}：${count} 题`);
...
export function renderReviewQueue(root, reviewQueue, totalDue = null) {
  if (!root) return;
  ...
  const count = document.getElementById("home-review-count");
  if (count) count.textContent = due.length ? `（${totalDue ?? due.length}）` : "";
  root.innerHTML = "";
  if (!due.length) {
    root.innerHTML = `<div class="review-empty"><svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="m8 12 3 3 5-6"/></svg><p>今日没有到期的复习题</p></div>`;
    return;
  }
  const list = document.createElement("ul");
  ...
  for (const item of due) {
    ...
    li.innerHTML = `
      ...
      <span class="review-meta">${escapeHtml(item.member)} · ${escapeHtml(item.platform)}${item.problemNumber ? ` ${escapeHtml(item.problemNumber)}` : ""}${item.difficultyRating || (item.difficulty && item.difficulty !== "未标注") ? ` · ${escapeHtml(difficultyText(item))}` : ""}</span>
    `;
...
// 复习队列 / 详情页共用的单题命令；服务端只接受复习字段，不让客户端回传整天日志。
export async function quickReviewAction(item, action, btn) {
  if (!currentUser) {
    alert("请先登录 GitHub");
    return;
  }
  ...
  try {
    const patch = action === "archive"
      ? { reviewStatus: REVIEW_STATUSES.ARCHIVED, reviewDue: undefined }
      : { reviewStatus: REVIEW_STATUSES.TODO, reviewDue: dueDateInDays(3) };
    await patchRecordReview(item.date, item.problemId, patch);
...
export function renderLogs(recordsRoot, logs, member, activeTagFilter, createLogCard, options = {}) {
  ...
  const filtered = logs.filter((log) =>
    ...
    (searchTerm === "" ||
      ...
      difficultyText(log).toLowerCase().includes(searchTerm))
  );
...
export function renderReviewBook(logs, member, status, tag, createLogCard) {
  const reviewLogs = logs.filter((log) => isReviewPlanned(log) && (member === "all" || log.member === member));
  ...
  const difficulty = document.getElementById("review-difficulty")?.value || "all";
  const dueFilter = document.getElementById("review-due")?.value || "all";
  const today = toDateString(new Date());
  const filtered = reviewLogs.filter((log) =>
    (status === "all" || log.reviewStatus === status) &&
    (tag === "all" || log.tags.includes(tag)) &&
    (difficulty === "all" || difficultyText(log) === difficulty) &&
    (dueFilter === "all" || (dueFilter === "due" && log.reviewDue && log.reviewDue <= today) || (dueFilter === "future" && log.reviewDue && log.reviewDue > today)) &&
    (searchTerm === "" ||
      log.problem.toLowerCase().includes(searchTerm) ||
      log.tags.some((t) => t.toLowerCase().includes(searchTerm)) ||
      log.platform.toLowerCase().includes(searchTerm) ||
  ...
  document.getElementById("review-all-count").textContent = String(reviewLogs.length);
  document.getElementById("review-todo-count").textContent = String(reviewLogs.filter((log) => log.reviewStatus === "todo").length);
  document.getElementById("review-mastered-count").textContent = String(reviewLogs.filter((log) => normalizeLearningState(log).reviewStatus === "archived").length);
  ...
  renderPagedList(root, sorted, (log) => {
    const overdue = log.reviewStatus === "todo" && log.reviewDue && log.reviewDue < today;
    const card = createLogCard(log, { className: overdue ? "review-overdue-card" : "" });
    const dueBadge = card.querySelector(".difficulty-badge");
    if (dueBadge) {
      dueBadge.className = `difficulty-badge due-status${overdue ? " overdue" : ""}${normalizeLearningState(log).reviewStatus === "archived" ? " mastered" : ""}`;
      if (normalizeLearningState(log).reviewStatus === "archived") dueBadge.textContent = "已结束安排";
      else if (!log.reviewDue) dueBadge.textContent = "待安排";
      else if (log.reviewDue === today) dueBadge.textContent = "今日到期";
      else if (overdue) dueBadge.textContent = "已到期";
      else dueBadge.textContent = `${Math.max(1, Math.ceil((new Date(`${log.reviewDue}T00:00:00`) - new Date(`${today}T00:00:00`)) / 86400000))} 天后到期`;
    }
    const actions = document.createElement("div");
    actions.className = "review-card-actions";
    const mastered = document.createElement("button");
    mastered.type = "button";
    mastered.className = "btn btn-primary";
    mastered.textContent = "✓ 结束复习";
    mastered.onclick = () => quickReviewAction(log, "archive", mastered);
    const snooze = document.createElement("button");
    snooze.type = "button";
    snooze.className = "btn btn-outline";
    snooze.textContent = "◷ 延后复习";
    snooze.onclick = () => quickReviewAction(log, "snooze", snooze);
    actions.append(mastered, snooze);
    const detail = document.createElement("a");
    detail.className = "btn btn-outline";
    detail.href = problemUrl(log);
    detail.textContent = "查看记录";
...
export function renderTrendChart(root, logs, start, end) {
  ...
  const chartWidth = Math.max(380, weeks.length * 76 + 44);
  ...
  const slotWidth = (plotRight - plotLeft) / weeks.length;
  const barWidth = Math.min(64, Math.max(18, slotWidth - 16));
  const tickCount = Math.min(4, maxCount);
  const labelStep = Math.max(1, Math.ceil(weeks.length / 6));

  let svg = `<svg class="trend-chart-svg" viewBox="0 0 ${chartWidth} ${chartHeight}" role="img" aria-label="周题量趋势图，显示 ${escapeHtml(start)} 至 ${escapeHtml(end)} 的每周训练题数">`;
  for (let i = 0; i <= tickCount; i += 1) {
    const value = Math.round((maxCount * i) / tickCount);
    const y = plotBottom - (plotHeight * i) / tickCount;
    svg += `<line class="trend-gridline" x1="${plotLeft}" y1="${y}" x2="${plotRight}" y2="${y}" />`;
    svg += `<text class="trend-axis-label" x="${plotLeft - 10}" y="${y + 4}" text-anchor="end">${value}</text>`;
  }
  svg += `<line class="trend-axis" x1="${plotLeft}" y1="${plotBottom}" x2="${plotRight}" y2="${plotBottom}" />`;

  weeks.forEach((week, i) => {
    const barHeight = week.count > 0 ? Math.max(5, (week.count / maxCount) * plotHeight) : 0;
    const x = plotLeft + i * slotWidth + (slotWidth - barWidth) / 2;
    const y = plotBottom - barHeight;
    const title = `${week.start.slice(5)} · ${week.count} 题`;
    if (barHeight) {
      svg += `<rect class="trend-bar" x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" rx="4" role="img"><title>${escapeHtml(title)}</title></rect>`;
      svg += `<text class="trend-value" x="${x + barWidth / 2}" y="${Math.max(13, y - 8)}" text-anchor="middle">${week.count}</text>`;
    }
    if (i % labelStep === 0 || i === weeks.length - 1) {
      svg += `<text class="trend-date-label" x="${x + barWidth / 2}" y="${plotBottom + 24}" text-anchor="middle">${escapeHtml(week.start.slice(5))}</text>`;
    }
  });

  svg += '</svg>';
  // 训练天数与构建期同口径：记录当天与确认区间的并集，并按所选范围裁剪。
  const activeDays = mergeTrainingDates(logs).filter((date) => date >= start && date <= end).length;
  const average = activeDays ? (logs.length / activeDays).toFixed(1) : '0';
  const peak = Math.max(...weeks.map((week) => week.count), 0);
  root.innerHTML = `${svg}<div class="trend-summary" aria-label="训练趋势汇总"><span><strong>${logs.length}</strong><small>训练题数</small></span><span><strong>${activeDays}</strong><small>训练天数</small></span><span><strong>${average}</strong><small>日均题数</small></span><span><strong>${peak}</strong><small>单周最高</small></span></div>`;
}
...
const LARGE_LIST_PAGE_SIZE = 40;

function renderPagedList(root, items, renderItem, emptyText) {
  ...
  let visible = 0;
  const appendPage = () => {
    root.querySelector(".page-load-more")?.remove();
    const fragment = document.createDocumentFragment();
    for (const item of items.slice(visible, visible + LARGE_LIST_PAGE_SIZE)) fragment.appendChild(renderItem(item));
    visible = Math.min(items.length, visible + LARGE_LIST_PAGE_SIZE);
    if (visible < items.length) {
      const more = document.createElement("button");
      more.type = "button";
      more.className = "btn btn-outline page-load-more";
      more.textContent = `加载更多（剩余 ${items.length - visible} 条）`;
...
export function renderAnalysis(logs, member, start, end, recent30, createLogCard) {
  ...
  const platform = document.getElementById("analysis-platform")?.value || "all";
  const difficulty = document.getElementById("analysis-difficulty")?.value || "all";
  const selectedTag = document.getElementById("analysis-tag")?.value || "all";
  const filtered = logs.filter((log) => {
    const memberMatches = member === "all" || log.member === member;
    const inRange = (!start || log.date >= start) && (!end || log.date <= end);
    const facetsMatch = (platform === "all" || log.platform === platform) &&
      (difficulty === "all" || difficultyText(log) === difficulty) &&
      (selectedTag === "all" || log.tags.includes(selectedTag));
    const termMatches = !searchTerm ||
      log.problem.toLowerCase().includes(searchTerm) ||
      log.member.toLowerCase().includes(searchTerm) ||
      (log.summary && log.summary.toLowerCase().includes(searchTerm)) ||
      log.tags.some((t) => t.toLowerCase().includes(searchTerm)) ||
      log.platform.toLowerCase().includes(searchTerm) ||
      (log.problemNumber && log.problemNumber.toLowerCase().includes(searchTerm)) ||
      log.difficulty.toLowerCase().includes(searchTerm);
    return memberMatches && inRange && facetsMatch && termMatches;
  });
  ...
  const todo = filtered.filter((log) => isReviewTodo(log)).length;
  const archived = filtered.filter((log) => normalizeLearningState(log).reviewStatus === "archived").length;
  ...
  renderCountMap(document.getElementById("analysis-review-stats"), { "待复习": todo, "已结束安排": archived }, "该时间范围暂无复习安排。");

  const recordsRoot = document.getElementById("analysis-records");
  renderPagedList(recordsRoot, filtered, (log) => {
...
export function renderMemberPage(logs, member, heatmap, recent30, createLogCard) {
  ...
  document.getElementById("member-breadcrumb").textContent = member || "成员";
  document.getElementById("profile-avatar").textContent = Array.from(member || "成员")[0];
  const memberReview = memberLogs.filter((log) => log.reviewStatus === "todo").length;
  document.getElementById("member-review-count").textContent = String(memberReview);
  let streak = 0;
  const streakDate = new Date();
  const memberCounts = heatmap.byMember[member] || {};
  if (!memberCounts[toDateString(streakDate)]) streakDate.setDate(streakDate.getDate() - 1);
  while (memberCounts[toDateString(streakDate)]) { streak += 1; streakDate.setDate(streakDate.getDate() - 1); }
  document.getElementById("member-streak").textContent = `${streak} 天`;
  ...
  const vitality = heatmap.vitalityByMember?.[member];
  document.getElementById("member-vitality-total").textContent = (vitality?.total || 0).toFixed(1);
  renderVitalityChart(document.getElementById("member-vitality-chart"), vitality?.daily, { total: vitality?.total });
  document.getElementById("member-vitality-details").innerHTML = memberVitalityDetailsHtml(vitality);
  renderHeatmap(heatmapRoot, counts, member, [member], heatmap.byMember, heatmap.valueByMember?.[member], heatmap.valueByMember);

  const recentRoot = document.getElementById("member-recent-list");
  recentRoot.innerHTML = memberLogs.slice(0, 5).map((log) => { const state = normalizeLearningState(log); const label = state.reviewStatus === "todo" ? "待复习" : state.masteryStatus === "mastered" ? "自评已掌握" : "已记录"; return `<a class="member-recent-row" href="${problemUrl(log)}"><strong>${escapeHtml(log.problemNumber || "题目")}</strong><span>${escapeHtml(log.problem)}</span><time>${escapeHtml(log.date)}</time><span class="review-chip ${state.reviewStatus === "todo" ? "todo" : "mastered"}">${label}</span></a>`; }).join("");
  const tagCounts = {};
  const difficultyCounts = {};
  const platformCounts = {};
  for (const log of memberLogs) {
    for (const tag of log.tags) tagCounts[tag] = (tagCounts[tag] || 0) + 1;
    const label = difficultyText(log);
    difficultyCounts[label] = (difficultyCounts[label] || 0) + 1;
    platformCounts[log.platform || "其他"] = (platformCounts[log.platform || "其他"] || 0) + 1;
  }
  renderCountMap(document.getElementById("member-difficulty"), difficultyCounts, "暂无难度数据");
  renderCountMap(document.getElementById("member-platforms"), platformCounts, "暂无平台数据");
  document.getElementById("member-tags").innerHTML = Object.entries(tagCounts).sort((a,b) => b[1]-a[1]).slice(0,10).map(([tag,count]) => `<a class="tag-chip member-tag" href="/tags/${encodeURIComponent(tag)}/">${escapeHtml(tag)} <span>${count}</span></a>`).join("");
  document.getElementById("member-platform-icons").innerHTML = Object.entries(platformCounts).slice(0,4).map(([platform,count]) => `<span title="${escapeHtml(platform)} ${count} 题">${escapeHtml(platform.slice(0,2))}</span>`).join("");
  const trendEnd = memberLogs.length ? memberLogs[0].date : "";
  const trendStartDate = trendEnd ? new Date(`${trendEnd}T00:00:00`) : null;
  if (trendStartDate) trendStartDate.setDate(trendStartDate.getDate() - 29);
  const trendStart = trendStartDate ? toDateString(trendStartDate) : "";
  renderTrendChart(document.getElementById("member-trend"), memberLogs.filter((log) => log.date >= trendStart), trendStart, trendEnd);

  renderPagedList(root, memberLogs, (log) => createLogCard(log), "该队员暂无训练记录。");
}
...
export function renderJournal(journal, dataScope = "overview", navigation = {}) {
  ...
  for (const log of logs) {
    ...
    Object.assign(log, normalizeLearningState(log));
  }
  const archiveQuery = dataScope === "analysis"
    ? new URLSearchParams(window.location.search).get("q")?.trim() || ""
    : "";
  const analysisSearchInput = document.getElementById("analysis-search");
  if (dataScope === "analysis" && analysisSearchInput) {
    const queryChanged = analysisSearchInput.value.trim() !== archiveQuery;
    analysisSearchInput.value = archiveQuery;
    document.getElementById("global-search").value = archiveQuery;
    if (archiveQuery && queryChanged) {
  ...
  async function renderProblemPage(member, date, index) {
    ...
    document.getElementById("problem-back-member").href = "/";
    // The export and owner actions must also be wired on a direct, prerendered visit.
    const sequence = nextProblemDetailSequence();
    root.innerHTML = `<p class="eyebrow">题目详情</p><h1>加载中</h1><p class="hint">正在加载题目描述、题解和代码...</p>`;
    let log, problemDetailHtml;
    try {
      const force = forceProblemDetailRefresh;
      clearForceRefresh();
      [log, { problemDetailHtml }] = await Promise.all([
        loadProblemDetail(member, date, decodeURIComponent(index), force),
        import("./problem-detail.mjs"),
      ]);
    } catch (error) {
    ...
    root.innerHTML = problemDetailHtml(log, { sourceUrl, memberHref: memberUrl(member) });
    ...
    if (exportBar) {
      ...
      exportBar.open = false;
    }

    // 本人记录提供「编辑此题 / 标记已掌握 / 顺延 +3」快捷操作（登录后且为本人记录时）
    if (currentUser && currentUser.member === log.member) {
      const editActions = root.querySelector("[data-problem-edit]");
      const reviewActions = root.querySelector("[data-problem-review]");
      editActions.hidden = false;
      reviewActions.hidden = false;
      const makeAction = (symbol, label, handler, title = "") => {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "btn btn-outline btn-sm review-detail-action";
        btn.innerHTML = `${icon(symbol)}${escapeHtml(label)}`;
        if (title) btn.title = title;
        btn.addEventListener("click", () => handler(btn));
        return btn;
      };
      editActions.append(
        makeAction("pen", "编辑此题", async () => {
          try {
      ...
      reviewActions.append(
        makeAction("check", "结束复习", (btn) => quickReviewAction(log, "archive", btn), "结束本题复习安排（不改变完成结果或掌握自评）"),
        makeAction("clock", "顺延 +3", (btn) => quickReviewAction(log, "snooze", btn), "复习日期顺延 3 天"),
      );
    }
  ...
  function render(member) {
    ...
    if (totalBadge && journal.totalLogs != null) {
      totalBadge.textContent = String(journal.totalLogs);
      totalBadge.classList.remove("loading-value");
    }
    // 活力指数徽章：与题数并排，按当前筛选成员显示
    const vitalityBadge = document.getElementById("site-vitality-badge");
    if (vitalityBadge) {
      const scope = member === "all" ? null : journal.vitality?.[member];
      const value = scope ? scope.total : journal.totalVitality;
      if (value != null) {
        vitalityBadge.textContent = Number(value).toFixed(1);
        vitalityBadge.classList.remove("loading-value");
      }
    }
    renderStats(member, recent30, member === "all" ? heatmap.all : (heatmap.byMember[member] || {}));
    renderHeatmap(
      document.getElementById("heatmap"),
      member === "all" ? heatmap.all : (heatmap.byMember[member] || {}),
      member,
      members,
      heatmap.byMember,
      member === "all" ? heatmap.valueAll : (heatmap.valueByMember?.[member] || {}),
      heatmap.valueByMember,
    );
    // 活力曲线：按当前筛选成员显示（当天聚合 + 当时的目标难度）
    const scope = member === "all" ? null : journal.vitality?.[member];
    renderVitalityChart(
      document.getElementById("vitality-chart"),
      scope ? scope.daily : journal.vitalityAllDaily,
      { total: scope ? scope.total : journal.totalVitality, targetRating: scope ? scope.targetRating : 0 },
    );
    renderReviewQueue(document.getElementById("review-queue"), journal.reviewQueue, journal.reviewQueueTotalDue);
    renderLogs(document.getElementById("records"), logs, member, activeTagFilter, createLogCard);
    if (dataScope === "analysis") {
  ...
  const platforms = [...new Set(logs.map((log) => log.platform).filter(Boolean))].sort();
  // 难度筛选项统一按 CF Rating 展示（与卡片上的徽章同口径）
  const difficulties = ratingOptions(logs.map((log) => log.difficultyRating)).map((rating) => `★ ${rating}`);
  populateSelect(document.getElementById("analysis-platform"), platforms);
  populateSelect(document.getElementById("analysis-difficulty"), difficulties);
  populateSelect(document.getElementById("analysis-tag"), allTags);
  populateSelect(document.getElementById("review-difficulty"), difficulties);
  document.getElementById("review-due").innerHTML = '<option value="all">全部时间</option><option value="due">已到期 / 今日</option><option value="future">未来到期</option>';

  async function renderRoute() {
    const parts = window.location.pathname.replace(/^\/+|\/+$/g, "").split("/");
    const routeKind = journalScopeForRoute(parts.join("/"));
  ...
  for (const id of ["analysis-platform", "analysis-difficulty", "analysis-tag"]) {
    document.getElementById(id).onchange = () => renderAnalysis(logs, analysisMember.value, document.getElementById("analysis-start").value, document.getElementById("analysis-end").value, recent30, createLogCard);
  }
  ...
  const reloadAnalysis = () => navigation.journal?.();
  document.getElementById("btn-analysis-search").onclick = () => analysisSearchHandler?.();
  document.getElementById("btn-analysis-reset").onclick = async () => {
    for (const id of ["analysis-member", "analysis-platform", "analysis-difficulty", "analysis-tag"]) document.getElementById(id).value = "all";
  ...
  if (analysisSearch && dataScope === "analysis") {
    analysisSearchHandler = async () => {
      const term = analysisSearch.value.trim();
      const url = new URL(window.location.href);
  ...
  analysisStart.value = analysisState.start;
  analysisEnd.value = analysisState.end;
  syncAnalysisDateRange();
  updateRangePresetState();
  if (dataScope === "analysis") {
    renderAnalysis(logs, analysisMember.value, analysisStart.value, analysisEnd.value, recent30, createLogCard);
  }
  for (const id of ["review-member", "review-status", "review-tag", "review-difficulty", "review-due"]) {
  ...
  async function render(kind, tag) {
    ...
    try {
      if (kind === "index") {
        ...
        const data = await ensureData();
        let entry = (data.tags || []).find((t) => t.tag === tag);
        if (!entry) {
          root.innerHTML = `<p class="hint">未找到标签「${escapeHtml(tag)}」。</p>`;
          return;
        }
        if (!Array.isArray(entry.records)) entry = await loadTagDetail(tag);
...
export function exportToPDF(log) {
  if (!log) return;
  return printMarkdownDocument((renderMarkdown) =>
    mdToPrintableHTML(buildMDContent(log), log.problem || "题目", renderMarkdown));
}
