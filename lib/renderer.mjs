...
export function renderMemberPage(logs, member, heatmap, recent30, createLogCard) {
  ...
  document.getElementById("member-tags").innerHTML = Object.entries(tagCounts).sort((a,b) => b[1]-a[1]).slice(0,10).map(([tag,count]) => `<a class="tag-chip member-tag" href="${tagHref(tag)}">${escapeHtml(tag)} <span>${count}</span></a>`).join("");
