...
export function renderReviewBook(logs, member, status, tag, createLogCard) {
  ...
  for (const log of sorted) {
    ...
    if (dueBadge) {
      dueBadge.className = `difficulty-badge due-status${overdue ? " overdue" : ""}${log.reviewStatus === "mastered" ? " mastered" : ""}`;
      if (log.reviewStatus === "mastered") dueBadge.textContent = "已掌握";
    ...
    mastered.textContent = "✓ 标记为已掌握";
    mastered.onclick = () => quickReviewAction(log, "mastered", mastered);
...
export function renderMemberPage(logs, member, heatmap, recent30, createLogCard) {
  ...
  recentRoot.innerHTML = memberLogs.slice(0, 5).map((log) => `<a class="member-recent-row" href="${problemUrl(log)}"><strong>${escapeHtml(log.problemNumber || "题目")}</strong><span>${escapeHtml(log.problem)}</span><time>${escapeHtml(log.date)}</time><span class="review-chip ${log.reviewStatus === "todo" ? "todo" : "mastered"}">${log.reviewStatus === "todo" ? "待复习" : "已记录"}</span></a>`).join("");
  const tagCounts = {};
  ...
  async function renderProblemPage(member, date, index) {
    ...
    if (currentUser && currentUser.member === log.member) {
      ...
      reviewActions.append(
        makeAction("check", "已掌握", (btn) => quickReviewAction(log, "mastered", btn), "把本题标记为已掌握（清除复习日期）"),
