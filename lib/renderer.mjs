...
export async function quickReviewAction(item, action, btn) {
  ...
  btn.textContent = "保存中...";
  try {
    const { problems, revision } = await loadDateLog(item.date);
    ...
    // 用刚刚读到的版本做条件写入：期间若有人改了这一天，服务端返回 409 而不是被静默覆盖。
    await saveDateLog(item.date, next, {}, revision);
...
export function renderAnalysis(logs, member, start, end, recent30, createLogCard) {
  ...
  renderPagedList(recordsRoot, filtered, (log) => {
    ...
    return wrapper;
  }, start && end ? "该时间范围暂无记录。" : "请选择开始和结束日期。");
...
export function renderJournal(journal, dataScope = "overview", navigation = {}) {
  ...
  if (dataScope === "analysis" && analysisSearchInput) {
    ...
    if (archiveQuery && logs.length && (queryChanged || !analysisStart.value || !analysisEnd.value)) {
      const dates = logs.map((log) => log.date).sort();
      analysisStart.value = dates[0];
      analysisEnd.value = dates.at(-1);
    }
  }
  const analysisState = {
  ...
  document.getElementById("btn-analysis-reset").onclick = async () => {
    ...
    analysisStart.value = month.start;
    analysisEnd.value = month.end;
