...
export function exportToMD(log) {
  ...
  if (log.description) {
    parts.push("## 题目描述", "", log.description, "");
  }
  if (log.takeaway) {
    parts.push("## 收获 / 题解", "", log.takeaway, "");
  }
  if (log.code) {
    parts.push("## 代码", "", "```cpp", log.code, "```", "");
  }
  ...
  a.href = url;
  a.download = `${log.member || "unknown"}-${log.date || "unknown"}-${(log.problem || "problem").replace(/[\\/:*?"<>|]/g, "_")}.md`;
