import { PLATFORMS, REVIEW_LABELS, toDateString } from "./constants.mjs";
...
export function renderAnalysis(logs, member, start, end, recent30, createLogCard) {
  ...
  if (exportMdBtn && !exportMdBtn._wired) {
    exportMdBtn._wired = true;
    exportMdBtn.addEventListener("click", () => {
      const selected = getSelectedLogs(filtered);
...
async function fetchFullLogs(selectedLogs) {
  if (selectedLogs.length > MAX_BATCH_EXPORT) {
    alert(`单次最多导出 ${MAX_BATCH_EXPORT} 题，当前选中 ${selectedLogs.length} 题。请缩小范围。`);
    return [];
  }
  const results = [];
  for (let i = 0; i < selectedLogs.length; i++) {
    const s = selectedLogs[i];
    const el = document.getElementById("export-status");
    if (el) el.textContent = `正在获取题目详情 ${i + 1}/${selectedLogs.length}...`;
    try {
...
function buildLatexContent(log) {
  if (!log) return "";
  const esc = (s) => String(s || "").replace(/[\\_{}&#%$~^]/g, (c) => "\\" + c).replace(/\n/g, "\\n\\allowbreak{}");
  const codeEsc = (s) => String(s || "").replace(/\\/g, "\\textbackslash{}").replace(/[{}]/g, (c) => "\\" + c).replace(/[_#%$~^&]/g, (c) => "\\" + c);
  const reviewLabel = { none: "非错题", todo: "待复习", mastered: "已掌握" }[log.reviewStatus] || "";

  const lines = [
    `\\section{${esc(log.problem || "题目")}}`,
    `\\begin{tabular}{|l|l|}`,
    `\\hline`,
...
export async function exportSelectedToMD(logs) {
  ...
  const fullLogs = await fetchFullLogs(logs);
...
<style>
  ...
  pre { background: #1e1e1e; color: #d4d4d4; padding: 16px; border-radius: 6px; overflow-x: auto; font-size: 0.875rem; line-height: 1.5; }
