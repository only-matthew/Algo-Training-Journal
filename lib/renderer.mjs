...
import { roadmapOverviewHtml, roadmapPhaseHtml, roadmapNodeHtml } from "./roadmap.mjs";
import { loadProblemDetail, loadRoadmapNode, ensureOverviewJournal, ensureFullJournal, forceProblemDetailRefresh, clearForceRefresh, problemDetailSequence, nextProblemDetailSequence } from "./data.mjs";
...
// 仅在页面包含 $...$ 公式时加载 KaTeX（~277KB JS + 字体）
...
export function renderRoadmap(roadmapData) {
  const { members } = roadmapData;
  ...
  async function render(kind, member) {
    ...
    try {
      if (kind === "overview") {
        tb.hidden = true;
        root.innerHTML = roadmapOverviewHtml(roadmapData, member);
      } else if (kind === "phase") {
        tb.hidden = false;
        root.innerHTML = roadmapPhaseHtml(roadmapData, view.phaseId, member);
      } else if (kind === "node") {
        tb.hidden = false;
  ...
  window.journalRouteRenderer = renderRoute;
  // 直链/深链首次加载时即按当前路径渲染（与 renderJournal 末尾 renderRoute() 一致），
  // 否则直接打开 /roadmap/<phase>/ 或 /roadmap/<phase>/<node>/ 时 view.kind 停留在
  // "overview"，切换成员会把总览内容渲染进阶段/题单页。
  renderRoute();
  return { render, renderRoute };
}
