...
export function renderReviewQueue(root, reviewQueue) {
  ...
  if (count) count.textContent = due.length ? `（${due.length}）` : "";
  root.innerHTML = "";
  if (!due.length) {
...
export function renderJournal(journal, dataScope = "overview", navigation = {}) {
  ...
  document.getElementById("btn-analysis-search").onclick = () => renderAnalysis(logs, analysisMember.value, analysisStart.value, analysisEnd.value, recent30, createLogCard);
  document.getElementById("btn-analysis-reset").onclick = () => {
    ...
    renderAnalysis(logs, "all", month.start, month.end, recent30, createLogCard);
  };
