...
// 仅依赖轻量共享模板与工具，全部为纯函数、无 DOM 访问或 Markdown 解析器。
...
import { nodeDifficultyTone, luoguDifficultyTone, evidenceStateTone } from "./roadmap-badges.mjs";
...
// 2. 知识点难度徽标：n 为 1-10 整数。
// 底色与前景由 lib/roadmap-badges.mjs 统一给出（并按 WCAG 对比度选字色），
// 内联样式保证深浅两套主题下都不会出现“深底深字”。
export function difficultyBadgeHtml(difficulty) {
  const tone = nodeDifficultyTone(difficulty);
  return `<span class="roadmap-difficulty" data-difficulty="${tone.level}" style="background:${tone.background};color:${tone.color}">难度 ${tone.level}</span>`;
}
...
// 状态色以 CSS 自定义属性（--evidence-light / --evidence-dark）下发，底色仍由样式表
// 用 color-mix 计算；未登记的状态返回空字符串，样式表会落到中性配色。
function evidenceStateStyle(state) {
  const tone = evidenceStateTone(state);
  return tone ? ` style="--evidence-light:${tone.light};--evidence-dark:${tone.dark}"` : "";
}
...
function trainingEvidenceBadgeHtml(node, member) {
  ...
  return `<span class="tag-chip roadmap-evidence" data-state="${escapeHtml(state)}"${evidenceStateStyle(state)}>🧭 ${escapeHtml(state)} · 训练 ${total}</span>`;
}
...
function trainingEvidenceSectionHtml(node, member) {
  ...
  return `<section class="roadmap-evidence-section" data-state="${escapeHtml(state)}"${evidenceStateStyle(state)}>
    <h3>🧭 训练证据</h3>
...
function orderedTopics(phases) {
  let routeIndex = 0;
  return phases.flatMap((phase, phaseIndex) => (phase.nodes || []).map((node, nodeIndex) => ({
    ...node,
    phaseId: phase.id,
    phaseTitle: phase.title,
    phaseGoal: phase.goal || phase.subtitle || "",
    phaseIndex,
    nodeIndex,
...
function roadmapViewTabsHtml() {
  return `<div class="roadmap-view-tabs" role="tablist" aria-label="知识地图视图">
    <button type="button" role="tab" id="roadmap-route-tab" aria-controls="roadmap-route-view" aria-selected="true" class="active" data-roadmap-view="route">按路线学习</button>
    <button type="button" role="tab" id="roadmap-index-tab" aria-controls="roadmap-index-view" aria-selected="false" data-roadmap-view="index">知识索引</button>
  </div>`;
}

function roadmapLearningRouteHtml(phases, member) {
  const topics = orderedTopics(phases);
  if (!topics.length) return `<p class="detail-empty">知识路线正在整理中。</p>`;
  const first = topics[0];
  const firstHref = `/roadmap/${encodeURIComponent(first.phaseId)}/${encodeURIComponent(first.id)}/`;
  const phaseBlocks = phases.map((phase, phaseIndex) => {
    const phaseTopics = topics.filter((node) => node.phaseId === phase.id);
    const rows = phaseTopics.map((node) => {
      const stats = statsFor(node.stats, member);
      const href = `/roadmap/${encodeURIComponent(phase.id)}/${encodeURIComponent(node.id)}/`;
      const routeNumber = String(node.routeIndex + 1).padStart(2, "0");
      return `<li class="learning-node${node.routeIndex === 0 ? " is-current" : ""}" data-roadmap-node="${escapeHtml(String(node.id))}">
        <a href="${href}" data-roadmap-node-link data-node-id="${escapeHtml(String(node.id))}" data-node-title="${escapeHtml(String(node.title || ""))}" data-node-href="${href}" data-node-position="${node.routeIndex + 1}" data-node-section="${node.nodeIndex + 1}" data-phase-id="${escapeHtml(String(phase.id))}" data-phase-title="${escapeHtml(String(phase.title || ""))}" data-phase-goal="${escapeHtml(String(phase.goal || phase.subtitle || ""))}" data-phase-number="${phaseIndex + 1}">
          <span class="learning-node-dot" aria-hidden="true"></span>
          <span class="learning-node-number">${routeNumber}</span>
          <span class="learning-node-copy"><strong>${escapeHtml(String(node.title || ""))}</strong><small>${escapeHtml(String(node.description || "查看本主题的学习资料与参考题。"))}</small></span>
          <span class="learning-node-facts">难度 ${escapeHtml(String(node.difficulty || "-"))} · ${stats.totalProblems} 道题</span>
          <span class="learning-node-current"${node.routeIndex === 0 ? "" : " hidden"}>当前位置</span>
          ${icon("right")}
        </a>
      </li>`;
    }).join("");
    return `<details class="learning-phase" data-learning-phase="${escapeHtml(String(phase.id))}"${phaseIndex === 0 ? " open" : ""}>
      <summary>
        <span class="learning-phase-number">${phaseIndex + 1}</span>
        <span class="learning-phase-copy"><small>阶段 ${phaseIndex + 1}</small><strong>${escapeHtml(String(phase.title || ""))}</strong><span>${escapeHtml(String(phase.subtitle || phase.goal || ""))}</span></span>
        <span class="learning-phase-count">${phaseTopics.length} 个知识点 ${icon("down")}</span>
      </summary>
      <ol class="learning-node-list">${rows}</ol>
    </details>`;
  }).join("");
  const total = topics.length;
  const progress = Math.max(2, Math.round(100 / total));
  return `<div class="learning-resume" data-learning-resume>
      <span class="learning-resume-icon">${icon("book")}</span>
      <span><small>从上次的位置继续</small><strong>继续学习：<span data-current-node-title>${escapeHtml(String(first.title || ""))}</span></strong><em data-current-node-meta>阶段 1 · ${escapeHtml(String(first.phaseTitle || ""))} · 第 1 节</em></span>
      <a class="btn btn-primary" href="${firstHref}" data-current-node-link>继续学习 ${icon("right")}</a>
    </div>
    <div class="learning-route-layout" data-route-total="${total}">
      <div class="learning-phase-list">${phaseBlocks}</div>
      <aside class="learning-route-aside" aria-label="学习位置">
        <section class="learning-aside-card">
          <div class="learning-aside-heading"><span>${icon("book")}</span><div><small>学习位置</small><strong data-current-phase-title>阶段 1 · ${escapeHtml(String(first.phaseTitle || ""))}</strong></div></div>
          <p><strong data-current-position>第 1 / ${total} 个知识点</strong></p>
          <div class="learning-position-bar" aria-hidden="true"><span data-current-progress style="width:${progress}%"></span></div>
        </section>
        <section class="learning-aside-card">
          <div class="learning-aside-heading"><span>${icon("bookmark")}</span><div><small>本阶段目标</small><strong>学完这一阶段，你将能够</strong></div></div>
          <p data-current-phase-goal>${escapeHtml(String(first.phaseGoal || "建立当前阶段的核心算法能力。"))}</p>
        </section>
      </aside>
    </div>`;
}

function roadmapKnowledgeIndexHtml(phases, member) {
  const topics = orderedTopics(phases).sort((a, b) => String(a.title).localeCompare(String(b.title), "zh-CN"));
  ...
  const cardHtml = topics.map((node) => {
    ...
    return `<article class="knowledge-topic-card" data-topic="${escapeHtml([node.title || "", ...(node.tags || [])].join(" "))}" data-title="${escapeHtml(node.title || "")}" data-categories="${categoriesForTopic(node).join(" ")}" data-count="${stats.totalProblems}">
      ...
      ${tagsHtml((node.tags || []).slice(0, 3))}
      <div class="knowledge-topic-stats"><span>${icon("file")}<small>相关题目</small><b>${stats.totalProblems}</b></span><a href="${href}" aria-label="查看${escapeHtml(String(node.title || ""))}">${icon("right")}</a></div>
    </article>`;
  }).join("");
  ...
  return `<div class="roadmap-layout roadmap-index-layout"><nav class="topic-nav" aria-label="知识主题分类">${nav}</nav><div class="knowledge-index-main">
      <div class="knowledge-toolbar"><label class="search-control">${icon("search")}<input id="knowledge-search" type="search" aria-label="搜索知识主题" placeholder="搜索知识主题，如 DP、最短路、KMP…" /></label><div class="knowledge-toolbar-meta"><p id="knowledge-results" class="hint" role="status">共 ${topics.length} 个知识主题</p><label><span class="sr-only">知识主题排序</span><select id="knowledge-sort" aria-label="知识主题排序"><option value="name">按名称</option><option value="count">按相关题目</option></select></label></div></div>
      <div class="knowledge-topic-grid">${cardHtml}</div><p id="knowledge-empty" class="detail-empty" hidden>没有符合条件的知识主题，请更换分类或搜索词。</p>
    </div></div>`;
}

// 6. 总览视图：默认按课程顺序学习，知识索引用于按名称或分类查找。
export function roadmapOverviewHtml(roadmapData, member) {
  const phases = (roadmapData && roadmapData.phases) || [];
  return `<div class="roadmap-overview">
    ${roadmapViewTabsHtml()}
    <section id="roadmap-route-view" role="tabpanel" aria-labelledby="roadmap-route-tab" data-roadmap-panel="route">${roadmapLearningRouteHtml(phases, member)}</section>
    <section id="roadmap-index-view" role="tabpanel" aria-labelledby="roadmap-index-tab" data-roadmap-panel="index" hidden>${roadmapKnowledgeIndexHtml(phases, member)}</section>
  </div>`;
}
...
// 洛谷官方难度标签的色阶定义在 lib/roadmap-badges.mjs（唯一来源），
// 这里只负责取色并内联，保证深浅主题下都不会出现“深底深字”。
export function roadmapProblemCardHtml(problem) {
  ...
  const luoguTone = problem.difficulty ? luoguDifficultyTone(problem.difficulty) : null;
  const difficulty = luoguTone
    ? `<span class="roadmap-luogu-difficulty" data-level="${luoguTone.level}" style="background:${luoguTone.background};color:${luoguTone.color}">${escapeHtml(String(problem.difficulty))}</span>`
    : problem.difficulty
      ? `<span class="roadmap-luogu-difficulty">${escapeHtml(String(problem.difficulty))}</span>`
      : "";
  ...
  const doneItems = (problem.doneBy || [])
    .map((d) => {
      const state = normalizeLearningState(d);
      const chip = state.reviewStatus === "todo"
        ? `<span class="review-chip todo">待复习</span>`
        : state.masteryStatus !== "unknown"
          ? `<span class="review-chip mastered">${escapeHtml(MASTERY_LABELS[state.masteryStatus])}</span>`
          : "";
  ...
  const nameLink = url
    ...
    : escapeHtml(nameRaw);
  return `<div class="roadmap-problem-card" data-problem-name="${escapeHtml([problem.number, nameRaw, ...(problem.tags || [])].join(" "))}" data-problem-difficulty="${escapeHtml(problem.difficulty || "未标注")}" data-problem-source="${escapeHtml(problem.source || "")}" data-record-count="${(problem.doneBy || []).length}">
...
export function roadmapNodeHtml(nodeData, member) {
  ...
  // 题单来源（洛谷深入浅出 / 罗勇军 / 刘汝佳 / CF / 洛谷书系三本书…）：
  // 并入书系题目后一个节点可能有 5 种以上来源，给出筛选入口。
  const sources = [...new Set(problems.map(problem => String(problem.source || "").trim()).filter(Boolean))];
  return `<div class="roadmap-node detail-view">
    ...
    <section class="detail-section" id="node-problems"><div class="section-heading"><div><h2>${icon("book")}相关题目</h2><p class="hint">本主题的参考题，结合自己的进度选择练习。</p></div><div class="node-filters"><label class="sr-only" for="node-sort">题目排序</label><select id="node-sort"><option value="default">按题单顺序</option><option value="records">按队内记录数</option><option value="name">按题号排序</option></select>${sources.length > 1 ? `<label class="sr-only" for="node-source">题目来源</label><select id="node-source"><option value="all">全部来源</option>${sources.map(s => `<option>${escapeHtml(s)}</option>`).join("")}</select>` : ""}<label class="sr-only" for="node-difficulty">题目难度</label><select id="node-difficulty"><option value="all">全部难度</option>${difficulties.map(d => `<option>${escapeHtml(d)}</option>`).join("")}</select><label class="search-control">${icon("search")}<input id="node-search" type="search" aria-label="搜索本主题题目" placeholder="搜索本主题下的题目…" /></label></div></div><p id="node-result-count" class="hint" role="status">共 ${problems.length} 道题目</p><div class="roadmap-problem-list">${cards}</div><p id="node-empty" class="detail-empty" ${problems.length ? "hidden" : ""}>没有符合条件的题目。</p></section>
  </div>`;
}
...
export function tagIndexHtml(tagIndex) {
  ...
  const cards = tags
    .map((entry) => {
      ...
      return `<a class="tag-index-card" data-tag-name="${escapeHtml(tag)}" href="${tagHref(tag)}"><strong>${icon("tag")}${escapeHtml(tag)}</strong><span class="tag-index-meta">${recordCount} 条记录 · ${nodeCount} 个知识主题</span></a>`;
    })
    .join("");
