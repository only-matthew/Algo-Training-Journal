...
export function roadmapOverviewHtml(roadmapData, member) {
  const phases = (roadmapData && roadmapData.phases) || [];
  const topics = phases.flatMap((phase) => (phase.nodes || []).map((node) => ({ ...node, phaseId: phase.id })));
  ...
  const cardHtml = topics.map((node) => {
    ...
    return `<article class="knowledge-topic-card" data-topic="${escapeHtml(String(node.title || ""))} ${(node.tags || []).map(String).join(" ")}">
  ...
  const nav = topics.slice(0, 12).map((node) => `<a href="/roadmap/${encodeURIComponent(node.phaseId)}/${encodeURIComponent(node.id)}/">${icon(iconFor(String(node.title || "")))}${escapeHtml(String(node.title || ""))}</a>`).join("");
  return `<div class="knowledge-toolbar"><label class="search-control">${icon("search")}<input id="knowledge-search" type="search" placeholder="搜索知识主题（如：DP、最短路、KMP…）" /></label><label>排序：<select id="knowledge-sort"><option>按名称 A-Z</option><option>按相关题目</option></select></label></div>
    <div class="roadmap-layout"><nav class="topic-nav" aria-label="知识主题">${nav}</nav><div class="knowledge-topic-grid">${cardHtml}</div></div>`;
}
...
export function roadmapNodeHtml(nodeData, member) {
  ...
  return `<div class="roadmap-node">
    ...
    ${node.ref ? `<details class="roadmap-sources"><summary>参考资料</summary><p>${escapeHtml(String(node.ref))}</p></details>` : ""}
    <h3 class="roadmap-problems-title">参考题</h3>
...
export function tagPageHtml(entry) {
  ...
  const hint = `<p class="hint">${recordCount} 条训练记录 · ${nodes.length} 个知识主题关联</p>`;
    ...
    ? `<section class="roadmap-tag-section">
      ...
      <ul class="roadmap-node-list">
        ${nodes.map((n) => {
          ...
          return `<li><a class="roadmap-node-ref" href="${escapeHtml(href)}">${escapeHtml(String(n.nodeTitle ?? ""))}</a> · ${escapeHtml(String(n.phaseTitle ?? ""))}</li>`;
        }).join("")}
      </ul>
    </section>`
    : "";
...
export function tagIndexHtml(tagIndex) {
  ...
  const cards = tags
    .map((entry) => {
      ...
      return `<a class="tag-index-card" href="/tags/${encodeURIComponent(tag)}/"><strong>${icon("tag")}${escapeHtml(tag)}</strong><span class="tag-index-meta">${recordCount} 条记录 · ${nodeCount} 个知识主题</span></a>`;
    })
    .join("");
