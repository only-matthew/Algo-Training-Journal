...
export function roadmapOverviewHtml(roadmapData, member) {
  const phases = (roadmapData && roadmapData.phases) || [];
  const cards = phases.map((phase) => roadmapPhaseCardHtml(phase)).join("");
  return `<div class="roadmap-intro">
    <p>从主题、标签或题目进入。这里的题单用于查阅和选题，不是需要逐题完成的清单。</p>
  </div>
...
export function tagIndexHtml(tagIndex) {
  ...
  const cards = tags
    .map((entry) => {
      ...
      return `<a class="tag-index-card" href="/tags/${encodeURIComponent(tag)}/"><strong>${escapeHtml(tag)}</strong><span class="tag-index-meta">${recordCount} 条记录 · ${nodeCount} 个知识主题</span></a>`;
    })
    .join("");
