...
function statementSource(value) {
  ...
  if (!["manual", "codeforces-html", "luogu-mirror", "atcoder-html", "ai-summary"].includes(value.kind)) throw new TypeError("题面来源无效");
  if (value.url !== undefined) {
    string(value.url, "题面来源地址", 2048);
    let url;
    try { url = new URL(value.url); } catch { throw new TypeError("题面来源地址无效"); }
    // URL 必须与 kind 对应：官方题面只认 codeforces.com 的题目路径，镜像只认洛谷同题页
    // （CF 题的 `CF<场次><代号>`、AtCoder 题的 `AT_<任务 ID>`），AtCoder 官方页只认
    // atcoder.jp 的题目页。防止把任意 URL 写成来源。
    const https = url.protocol === "https:" && !url.username && !url.password && !url.port;
    const allowed = !https ? false
      : value.kind === "luogu-mirror"
        ? url.hostname === "www.luogu.com.cn" && /^\/problem\/(?:CF\d+[A-Z]\d*|AT_[A-Za-z0-9][A-Za-z0-9_]*)\/?$/.test(url.pathname)
        : value.kind === "atcoder-html"
          ? url.hostname === "atcoder.jp" && /^\/contests\/[A-Za-z0-9_-]+\/tasks\/[A-Za-z0-9_]+\/?$/.test(url.pathname)
          : url.hostname === "codeforces.com" && /^\/(?:problemset\/problem\/\d+\/[A-Z]\d*|(?:contest|gym)\/\d+\/problem\/[A-Z]\d*)\/?$/i.test(url.pathname);
