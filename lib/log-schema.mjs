...
export function metaFromProblems(problems, updatedAt) {
  return {
    schemaVersion: LOG_SCHEMA_VERSION,
    ...(typeof updatedAt === "string" && updatedAt ? { updatedAt } : {}),
    problems: problems.map(({ id, name, problem, platform, problemNumber, difficulty, tags, reviewStatus, reviewDue }) => {
...
export function problemStableKey(platform, problemNumber) {
  ...
  const n = String(problemNumber || "").trim().toUpperCase().replace(/\s+/g, "");
  if (!p || !n) return "";
  return `${p}|${n}`;
}
