...
function normalizeTags(value) {
  return [...new Set(splitTags(value).map((tag) => tag.trim()).filter(Boolean))].slice(0, LOG_LIMITS.tags);
}
...
function validateTags(value) {
  ...
  const tags = [...new Set(splitTags(value).map((tag) => tag.trim()).filter(Boolean))];
