 ...
 * 「已经选好但还没成功保存到仓库」的 PDF，让刷新或误关页面之后仍能继续提交。
 * 所有方法都返回状态而不抛异常：存储不可用（隐私模式、配额、旧浏览器）时表单必须
 * 仍能正常填写与提交——只是丢掉了本地恢复能力。
 */
export function createAttachmentStore({ indexedDB: idb = globalThis.indexedDB, dbName = DB_NAME, maxNewBytes = MAX_NEW_ATTACHMENT_BYTES } = {}) {
  ...
  return Object.freeze({
    /**
     * 读取某天已选好的附件。返回 `problemId -> { blob, fileName, sha256, bytes }`。
    ...
    async loadDate(memberId, date) {
      ...
      for (const [problemId, item] of Object.entries(record.items)) {
        // 逐条校验：IndexedDB 里的内容可能来自旧版本或被外部改写，坏一条不影响其余。
        if (item && item.blob instanceof Blob && typeof item.fileName === "string" && typeof item.sha256 === "string") {
    ...
    async save({ memberId, date, problemId, blob, fileName, sha256 }) {
      ...
      const record = await readRecord(memberId, date);
      const items = { ...(record?.items || {}), [problemId]: { blob, fileName, sha256, bytes: blob.size } };
      const total = Object.values(items).reduce((sum, item) => sum + (Number(item?.bytes) || 0), 0);
