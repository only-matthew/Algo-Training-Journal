...
export function projectReviewSchedule({ memberId, subjectKey, events = [], legacyReviews = [], dateMath } = {}) {
  ...
  if (explicit && Number(explicit.sequence) > lastAttemptSequence) {
    ...
    // With no effective attempt, explicit action, or legacy due date there is
    // nothing to schedule.  This case occurs after the only attempt is voided.
    state = legacy?.state || "archived";
