...
- 提交表单支持快速导入：输入 Codeforces / AtCoder 用户名一键拉取**最近 3 天**的 AC 记录（登录队员会自动预填自己的 CF 用户名，列表内可直接打开 CF AC 提交页复制源码，CF 提交页受反爬保护无法服务端自动抓取）；或粘贴洛谷题号自动补全**题名、官方难度与题目描述**。题目描述也可一键“抓取 CF 题面”：官方题面优先，被反爬拦下时自动退回洛谷同题镜像，来源会写进记录。**题面里的图片会随抓取一起下载并归档到仓库**（洛谷 CDN 与 Codeforces 图床的外链在站内被 CSP 拦下、本来显示不出来），与题面 PDF 走同一条保存链路：正文里的图片地址改写成仓库内的相对文件名，保存时随整日记录上传，构建期发布到站点。导入的 CF 英文标签会自动合并为中文标签（如 `graphs` → 图论），AtCoder 与洛谷不提供公开标签名称接口、导入后需在表单中手动补充标签；导入题目优先填入未填写的空题目位。
...
填写题目时可使用“快速导入”：登录后点击表单上方的“Codeforces AC 记录”或“洛谷题号”按钮，查询并勾选结果后一键回填为题目块。题目描述一栏也可点“抓取 CF 题面”：先取 Codeforces 官方英文题面，若被 Cloudflare 反爬拦下（机房出口常见 403「Just a moment」挑战页）则退回洛谷同题页（`CF<题号>`）兜底，镜像来源会在表单里标注，正文仍可在保存前人工修改。

题面里的图片在两条来源下都会被下载并归档：正文不再出现 `cdn.luogu.com.cn` / 题面图床的外链，而是仓库内的 `statement-<sha256>.<ext>`，保存后由构建期发布到站点（站点 CSP 只放行同源图片）。图片是否保留由正文决定——描述里仍然引用某个文件名才会保存它，用 AI 把题面概括成一句话之后，正文不再引用的图片会在下次保存时从仓库删除；取不到的图片保留原外链并提示。
