...
- 提交表单支持快速导入：输入 Codeforces / AtCoder 用户名一键拉取**最近 3 天**的 AC 记录（登录队员会自动预填自己的 CF 用户名，列表内可直接打开 CF AC 提交页复制源码，CF 提交页受反爬保护无法服务端自动抓取）；或粘贴洛谷题号自动补全**题名、官方难度与题目描述**。题目描述也可一键“抓取 CF 题面”：官方题面优先，被反爬拦下时自动退回洛谷同题镜像，来源会写进记录。导入的 CF 英文标签会自动合并为中文标签（如 `graphs` → 图论），AtCoder 与洛谷不提供公开标签名称接口、导入后需在表单中手动补充标签；导入题目优先填入未填写的空题目位。
