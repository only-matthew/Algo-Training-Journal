...
填写题目时可使用“快速导入”：登录后点击表单上方的“Codeforces AC 记录”或“洛谷题号”按钮，查询并勾选结果后一键回填为题目块。题目描述一栏也可点“抓取题面”：按当前平台与题号取题面——Codeforces 先取官方英文题面，若被 Cloudflare 反爬拦下（机房出口常见 403「Just a moment」挑战页）则退回洛谷同题页（`CF<题号>`）兜底；AtCoder 也先试官方英文题面，但 atcoder.jp 对机房出口整体 403（连首页都是，换请求头无效），因此实际取到的是洛谷的 `AT_<任务 ID>` 镜像页（多为日文原题；洛谷未收录 Typical90、JOI 等题目时提示无镜像）。来源与解析版本会在表单里标注，正文仍可在保存前人工修改。
...
- 题面抓取按平台取来源：Codeforces 官方页直取失败时退回洛谷同题镜像（镜像措辞可能与官方英文题面有出入，表单会提示核对）；AtCoder 只认「比赛_任务」形态的题号（如 `abc381_a`），官方页对机房出口整体 403（Cloudflare Workers 实测，换请求头无效），实际来源是洛谷的 `AT_<任务 ID>` 镜像页（多为日文原题，洛谷未收录 Typical90、JOI 等题目）。两个来源都没取到时按 `blocked` 降级，不做重试与绕过。
