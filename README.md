...
- 独立提交页 `/submit/` 支持快速导入：输入 Codeforces / AtCoder 用户名一键拉取**最近 3 天**的 AC 记录（登录队员会自动预填自己的 CF 与 AtCoder 用户名，列表内可直接打开 AC 提交页复制源码，CF 提交页受反爬保护无法服务端自动抓取）；或粘贴洛谷题号自动补全**题名、官方难度与题目描述**。洛谷页面同时提供原文 `content` 和中文本地化 `contenu` 时默认采用中文版本（如 P2895），中文为空才回退原文。Codeforces 的导入带官方算法标签；AtCoder 的标签取自洛谷镜像（ABC/ARC/AGC 常见题可用，洛谷未收录的题目需手填）；洛谷题目导入当前只解析数字标签 ID，标签需手填。题目描述也可一键“抓取题面”，按当前平台与题号取题面：Codeforces 官方英文题面优先、被反爬拦下时自动退回洛谷同题镜像；AtCoder 同样官方页优先，但 atcoder.jp 对机房出口整体返回 403（Cloudflare Workers 取不到），实际来源是洛谷的 `AT_<任务 ID>` 镜像页（多为日文原题，表单提示核对；洛谷未收录的题目会明确提示没有镜像）。来源会写进记录。**题面里的图片会随抓取一起下载并归档到仓库**（洛谷 CDN、Codeforces 与 AtCoder 图床的外链在站内被 CSP 拦下、本来显示不出来），与题面 PDF 走同一条保存链路：正文里的图片地址改写成仓库内的相对文件名，保存时随整日记录上传，构建期发布到站点。导入的 CF 英文标签会自动合并为中文标签（如 `graphs` → 图论）；导入题目优先填入未填写的空题目位。
- 描述、心得和代码分文件保存，Markdown 或代码内容不会干扰其他字段解析；三块内容都直接显示在表单里，代码块可手动折叠。
...
填写题目时可使用“快速导入”：登录后进入 `/submit/`，点击工具栏中的“CF 导入”“洛谷导入”或“AtCoder 导入”，查询并勾选结果后一键回填为题目块。洛谷导入优先读取页面内嵌 JSON 的中文 `contenu`，没有可用中文正文时再读取原文 `content`。题目描述一栏也可点“抓取题面”：按当前平台与题号取题面——Codeforces 先取官方英文题面，若被 Cloudflare 反爬拦下（机房出口常见 403「Just a moment」挑战页）则退回洛谷同题页（`CF<题号>`）兜底；AtCoder 也先试官方英文题面，但 atcoder.jp 对机房出口整体 403（连首页都是，换请求头无效），因此实际取到的是洛谷的 `AT_<任务 ID>` 镜像页（多为日文原题；洛谷未收录 Typical90 等题目时提示无镜像）。来源与解析版本会在表单里标注，正文仍可在保存前人工修改。

洛谷没收录的 AtCoder 题（Typical90 等）用同一栏里的「从 AtCoder 页面导入（洛谷没收录的题用这条）」：把小书签拖到书签栏 → 在 AtCoder 题目页点一下（复制整页源码）→ 回到表单粘贴 → 点「解析并填入」。这一步走的是你自己的浏览器网络，因此不受机房 IP 拦截与跨域限制；不想装小书签也可以在该页 `Ctrl+U` 打开源码后 `Ctrl+A`、`Ctrl+C`。

题面里的图片在三条来源下都会被下载并归档：正文不再出现 `cdn.luogu.com.cn` / 题面图床的外链，而是仓库内的 `statement-<sha256>.<ext>`，保存后由构建期发布到站点（站点 CSP 只放行同源图片）。图片是否保留由正文决定——描述里仍然引用某个文件名才会保存它，用 AI 把题面概括成一句话之后，正文不再引用的图片会在下次保存时从仓库删除；取不到的图片保留原外链并提示。
...
- `meta.json`：题目永久 ID、名称、题号、平台、难度、标签、错题状态、复习日期（`reviewDue`，可选，格式 `YYYY-MM-DD`），以及该打卡记录的 `updatedAt`（最后更新时间，ISO 格式时间戳）。修复过题号的历史记录还会带 `problemNumberLegacy`（题号原值，仅供回滚参考）；题面附件与 `statementImages`（归档题面图片）也登记在这里。
- `N-desc.md`：第 N 道题的题目描述；正文里的题面图片写成本目录下的显式相对路径 `./statement-<sha256>.<ext>`（GitHub 页面与站内渲染器都能解析，站点构建时改写成站点地址）。
- `N-takeaway.md`：第 N 道题的心得或题解。
- `N-solution.cpp`：第 N 道题的 C++ 代码。
- `N-statement-<sha256>.pdf`：归档的题面 PDF（由 v2 附件接口写入）。
- `statement-<sha256>.<ext>`：归档的题面图片（png/jpg/gif/webp，由抓取流程带入、随保存写入；文件名即内容哈希，同一天内同图只存一份）。
...
├── src/                           # 浏览器端源码与静态资源
│   ├── app.js                     # 应用入口：认证、路由、渲染和表单初始化
│   ├── problem-page.js            # 独立题目页入口
│   ├── index.html                 # 单页应用页面骨架
│   ├── style.css                  # 组件、主题与响应式样式
│   └── assets/                    # 山水素材及拆分维护的补充样式
├── lib/
...
- **洛谷没有收录的 AtCoder 题目（如 Typical90）用「从 AtCoder 页面导入」兜底**：AtCoder 拒绝服务器直连，而浏览器直接读取又受跨域限制（AtCoder 不返回 `Access-Control-Allow-Origin`），唯一能拿到官方页的是跑在 `atcoder.jp` 上的代码。表单里提供一个可拖到书签栏的「抓取 AtCoder 题面」小书签：在题目页点一下会复制整页源码，粘回表单后由服务端用**同一个解析器**处理（含 `og:url` 校验，粘错题会判 `parse-failed`，且这条路径不请求任何上游）。正文是官方原文，日文题会标注；题面图片在 `img.atcoder.jp` 同样够不到，退回外链并提示（站点 CSP 只放行同源图片，因此那些图不会显示）。
- AtCoder 的算法标签同样只能从洛谷镜像拿：它是洛谷社区给这些题标的标签（不是 AtCoder 官方标签），简单题常常一个都没有，洛谷没收录的题目更是完全没有；标签会先按 `lib/luogu-tag-map.mjs` 尽量映射到站内规范标签（262 个洛谷算法标签里 223 个能直接对上，站内没有的成熟技巧如「莫队」「笛卡尔树」保留原名，分类名与「语言入门」语法标签丢弃，超过 30 字符的丢弃）。导入会因此多花一到几次请求（一场比赛一次列表页），拿不到只是没有标签，不影响导入与题面。
- 题面图片在抓取时一并下载（单张 ≤1 MiB、每题 ≤10 张、一次保存新增合计 ≤2 MiB）；取不到或类型不受支持（如 SVG）时保留原外链并提示，正文本身照常可用。站点 CSP 只放行同源图片，所以未归档的外链在站内不会显示。
