...
- 提交表单支持快速导入：输入 Codeforces / AtCoder 用户名一键拉取**最近 3 天**的 AC 记录（登录队员会自动预填自己的 CF 与 AtCoder 用户名，列表内可直接打开 AC 提交页复制源码，CF 提交页受反爬保护无法服务端自动抓取）；或粘贴洛谷题号自动补全**题名、官方难度与题目描述**。Codeforces 的导入带官方算法标签，AtCoder 的标签取自洛谷镜像（ABC/ARC/AGC 常见题可用，洛谷未收录的题目需手填）。题目描述也可一键“抓取题面”，按当前平台与题号取题面：Codeforces 官方英文题面优先、被反爬拦下时自动退回洛谷同题镜像；AtCoder 同样官方页优先，但 atcoder.jp 对机房出口整体返回 403（Cloudflare Workers 取不到），实际来源是洛谷的 `AT_<任务 ID>` 镜像页（多为日文原题，表单提示核对；洛谷未收录的题目会明确提示没有镜像）。来源会写进记录。**题面里的图片会随抓取一起下载并归档到仓库**（洛谷 CDN、Codeforces 与 AtCoder 图床的外链在站内被 CSP 拦下、本来显示不出来），与题面 PDF 走同一条保存链路：正文里的图片地址改写成仓库内的相对文件名，保存时随整日记录上传，构建期发布到站点。导入的 CF 英文标签会自动合并为中文标签（如 `graphs` → 图论），AtCoder 与洛谷不提供公开标签名称接口、导入后需在表单中手动补充标签；导入题目优先填入未填写的空题目位。
