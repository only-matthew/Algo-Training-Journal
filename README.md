...
- 提交表单支持快速导入：输入 Codeforces / AtCoder 用户名一键拉取**最近 3 天**的 AC 记录（登录队员会自动预填自己的 CF 与 AtCoder 用户名，列表内可直接打开 CF AC 提交页复制源码，CF 提交页受反爬保护无法服务端自动抓取）；或粘贴洛谷题号自动补全**题名、官方难度与题目描述**。题目描述也可一键“抓取题面”，按当前平台与题号取官方题面：Codeforces 官方英文题面优先、被反爬拦下时自动退回洛谷同题镜像，AtCoder 直接取官方英文题面（少数老题只有日文原题时会明确提示），来源会写进记录。**题面里的图片会随抓取一起下载并归档到仓库**（洛谷 CDN、Codeforces 与 AtCoder 图床的外链在站内被 CSP 拦下、本来显示不出来），与题面 PDF 走同一条保存链路：正文里的图片地址改写成仓库内的相对文件名，保存时随整日记录上传，构建期发布到站点。导入的 CF 英文标签会自动合并为中文标签（如 `graphs` → 图论），AtCoder 与洛谷不提供公开标签名称接口、导入后需在表单中手动补充标签；导入题目优先填入未填写的空题目位。
...
填写题目时可使用“快速导入”：登录后点击表单上方的“Codeforces AC 记录”或“洛谷题号”按钮，查询并勾选结果后一键回填为题目块。题目描述一栏也可点“抓取题面”：按当前平台与题号取官方题面——Codeforces 先取官方英文题面，若被 Cloudflare 反爬拦下（机房出口常见 403「Just a moment」挑战页）则退回洛谷同题页（`CF<题号>`）兜底；AtCoder 直接取官方英文题面（`atcoder.jp/contests/<比赛>/tasks/<任务>`，少数老题只有日文原题会明确提示），来源与解析版本会在表单里标注，正文仍可在保存前人工修改。
...
- **AtCoder**：通过 AtCoder Problems 公开 API（kenkoooo.com）拉取该用户**最近 3 天内的 AC 记录**（分页覆盖窗口、按题目去重），返回题名、题号与 Rating；AtCoder 不提供公开标签名称接口，导入后需在表单中手动补充标签。题面不在导入时批量抓取（AC 列表不阻塞等待题面），导入后按题点「抓取题面」即可从官方题目页取回英文题面。
...
单题题面抓取走 `POST /api/problem-statement`（`platform` 为 `Codeforces` 或 `AtCoder`），与导入接口共用登录会话、CSRF 与同一套限流。响应携带 `source.kind`（`codeforces-html` / `luogu-mirror` / `atcoder-html`）与 `parserVersion`；保存时由 `lib/problem-enrichment-schema.mjs` 按 kind 校验来源地址（AtCoder 只认 `atcoder.jp/contests/<比赛>/tasks/<任务>`），图片字节随响应回传后由表单交给 v2 保存链路。
...
- 题面抓取按平台取官方页：Codeforces 直取失败时退回洛谷镜像（镜像措辞可能与官方英文题面有出入，表单会提示核对）；AtCoder 只认「比赛_任务」形态的题号（如 `abc381_a`），少数老题没有英文题面，抓到的日文原题会在表单里明确标注。AtCoder 对异常流量会限流（429），拿不到时按 `blocked` 降级，不做重试与绕过。
